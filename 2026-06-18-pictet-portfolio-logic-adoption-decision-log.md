...
## §S13.43 옵션 기반 risk on-off 레짐 신호 — 사전점검 사전등록 (2026-08-14, 측정 전)

**출처**: 사용자 지시 2026-08-14 — "VIX 텀 프리미엄 관련 지수 + 다양한
주식/인덱스 옵션 지표를 조합해 risk on-off를 **미리** 판단하는 레짐 분석기".
AskUserQuestion으로 4결정 확정: ① 용도는 **사전점검부터**(용도 결정은 결과
후), ② 타깃은 **3-타깃 매트릭스**, ③ 신호는 **사전등록 고정세트 등가중**,
④ 접근은 **순수 read-only 스크립트**(production·config 무접촉 — parity
비해당).

**이력 제약(사전 기록)**: 레짐 엔진(regime_v2, full ΔIR −0.048)·μ-디리스킹
(§S13.34 구조적 no-op)·진입 게이트(§S13.39 FAIL, 방향 반대)·TE-캡 조건화
(§S13.22 FAIL) — risk-off "수익률 경로" 액션 채널 전부 실패. 유일 성공
전달 경로는 Σ(§S13.41). §S13.17 실증: S0는 stress에서 안 잃음. 따라서
타깃별 PASS의 후속 의미를 사전 고정한다: **T2(변동성) PASS → 레짐 조건부
Σ 스케일링 후속 사전등록 후보. T1(bm 하락)만 PASS → 모니터링 도구
후보(액션 없음). T3(북 active) PASS → §S13.17과의 정합 진단 후 해석.**

**합성 신호 (사전등록 고정 6성분, 등가중, + = risk-off, 스윕·사후 재가중
금지)**:

| # | 성분 | 정의 | 원천 |
|---|---|---|---|
| 1 | `vix_tp_z` | Z252(VIX − VIX3M) — 텀 프리미엄 축소/역전 = risk-off | Factor_PX_LAST |
| 2 | `vix_z` | Z252(VIX) | Factor_PX_LAST |
| 3 | `skew_idx_z` | Z252(SKEW 인덱스) | Factor_PX_LAST |
| 4 | `spx_iv_ts_z` | `iv_term_structure_z` 시트 SPX 열 | §S13.37 시트 |
| 5 | `cs_iv_breadth` | `iv30_z` 유니버스 단면 중앙값 | §S13.37 시트 |
| 6 | `cs_skew_chg` | Z252(`downside_skew_chg_5d` 단면 평균) | §S13.37 시트 |

- composite = 가용 성분 평균(**최소 4성분** 가용 시점부터 표본 시작).
- `comp_z` = composite의 **expanding z**(번인 252BD, point-in-time —
  실시간 계산 가능성 유지). risk-off 상태 = comp_z > +1.
- 성분별 개별 리드 테이블은 **진단으로만** 출력(비액션) — 판정은 합성
  하나로만.

**타깃 3종 (각 신호일 익일부터 향후 21BD)**: T1 = bm 누적수익률(창내
최대낙폭은 진단), T2 = bm 실현변동성(연율), T3 = 북 active 누적수익률.
bm·북 시리즈 = 현 production `arm_s13_41a_optvol_cov/backtest_result.pkl`의
`benchmark_returns`·`active_returns`(08-12 빈티지 기준선 1.6623 산출물).

**게이트**:
- **P0 (분리력)**: 비중첩 21BD 윈도우 시점(≈89개)에서 comp_z 상위 20% vs
  나머지의 타깃 평균 차이 **t ≥ 2 AND 3분할 부호 일관**. 방향 사전 고정:
  T1·T3 하락, T2 상승. 타깃별 독립 판정.
- **P1 (증분 선행성 — 핵심)**: 일별 중첩 표본에서 타깃 ~ [직전 21BD bm
  실현변동성, 직전 21BD bm 누적수익률] 통제 회귀 + comp_z, Newey-West
  (lag 21) **t ≥ 2**, 방향 P0과 동일. 통과 못 하면 "미리"가 아니라 이미
  일어난 것의 재표현 — §S13.42의 vs-지속성 관문과 동일 정신.
- **P2 (실용 발화율)**: risk-off 상태 점유율 **5~30%** AND 평균 지속
  **≥ 5BD**(§S13.39의 6건/7.7년 무의미 발화 재발 방지).
- **종합**: 타깃별 (P0∧P1) 판정 + 전역 P2. 어떤 타깃도 P0∧P1을 못 넘거나
  P2 FAIL → **SHELVE**(레짐 축 소진 기록). PASS 타깃이 있으면 위 사전
  고정 의미에 따라 **후속은 별도 사전등록**(이 점검은 arm이 아님).

**절차/검증 의무**: `scripts/precheck_s13_43_regime.py`(read-only, 단일
foreground, 백테스트 없음 — 순수 시계열 연산) + 단위테스트(합성·전진창·
NW t·상태 지속 로직, fixture 없는 plain 함수). 실행 선행 검증: VIX·VIX3M·
SKEW 컬럼 존재·가용 시작일 보고, 시트 SPX 열 존재 확인, 성분별 결측률
보고(§4.3 유사 의무), **워크북 mtime 보고** — 08-12 13:45:41과 다르면
신호(현 워크북) vs 북 시리즈(08-12 빈티지) 빈티지 괴리를 명시 기록.
인벤토리: read-only 사전점검 미산입(**465 불변**, §S13.7 전례). 산출물
`outputs/s13_43_regime_precheck/summary.json`.

**사전 관측 기록**: VIX9D/VIX6M/COR1M/3M은 워크북 원천에 존재하나
미배선(§S13.34 확인) — 고정세트에 불포함, 재도전 시 새 사전등록.
§S13.42가 종목 레벨에서 "IV 레벨 이후 skew/term 증분 0"을 실증했으므로
성분 4·6의 독자 기여는 낮게 기대하며, 등가중 합성이라 판정에는 무해.
T2는 vol 지속성이 강해 P1 바가 실질 관문이다. SHELVE가 기본 기대치.

### §S13.43 사전점검 실측 (2026-08-14) — P2 FAIL(T2만 P0·P1 PASS) → SHELVE, 단 변동성 리드는 실재

실행: `scripts/precheck_s13_43_regime.py`(단위테스트 6건, 스위트 526 PASS),
산출물 `outputs/s13_43_regime_precheck/summary.json`. 워크북 08-12 빈티지
**불변 확인** — 스크립트 mtime 표기는 UTC `04:45:41`(= 결정 로그 관용
KST 13:45:41), §S13.42 summary.json의 동일 필드와 문자열 일치로 대조
확정. 표본: 일별 1895개(2018-12-26~2026-07-13, production pkl의 bm·active
기간), 비중첩 91개. 커버리지 6성분 92.5~100%, 최초 유효 2014-05~07.

- **T2 (bm 향후 21BD 실현변동성): P0 PASS**(t +2.95, 3분할 전부 양
  [+0.180/+0.104/+0.075] — risk-off 상위 20% 시점의 향후 변동성 25.4%
  vs 나머지 13.7%) **AND P1 PASS**(NW t +2.30, coef +0.040 — 직전 21BD
  실현변동성·수익률 통제 후 증분). **합성 레짐 신호의 변동성 선행
  정보는 실재한다.**
- **T1 (bm 수익률): P0·P1 FAIL**(t −0.26 / +0.23). risk-off 상위
  구간의 향후 21BD bm 수익이 오히려 +2.06%(나머지 +1.67%) — **시장
  하락은 미리 못 맞힌다**(§S13.39 "극단 시그널 뒤 수익 양" 재확인).
  창내 최대낙폭 진단: risk-off −6.1% vs 전체 −4.0% — 낙폭 심화는
  변동성 확대 경로의 부산물이지 방향 예측이 아님.
- **T3 (북 active): P0·P1 FAIL**(t +0.51 / +0.45) — §S13.17("S0는
  stress에서 안 잃음")과 정합. 레짐으로 북 손익은 미리 알 수 없음.
- **P2 FAIL**: comp_z>+1 점유율 10.6%(밴드 내 OK)이나 **평균 지속
  3.1BD < 5BD**(64 스펠) — 상태가 깜빡여 레짐이라 부를 수 없음.
  사전등록 종합식(전역 P2)에 따라 **SHELVE**.
- 성분 진단(비액션, T2 NW t): cs_iv_breadth +2.84 / vix_z +2.71 /
  vix_tp_z +2.25 / spx_iv_ts_z +2.02 / cs_skew_chg +1.69 — 리드가
  광범위. 예외는 skew_idx_z(CBOE SKEW): 전 타깃 음(T3 −2.23) —
  SKEW 인덱스는 리드 부재·역방향.
- **사전 고정 의미 적용**: T1 FAIL → "하락 사전 탐지" 모니터링 도구
  근거 없음. T2 PASS의 한계 명시 — P1 통제는 실현변동성 지속성뿐이며,
  **현 production 변동성 모델(B: lt+iv30_z, §S13.41)이 이미 보유한 정보
  대비 증분은 미검증**. 재도전은 §S13.42 P0'(vs B 공통표본) 프레임 +
  상태 정의(스무딩/히스테리시스 등) 별도 사전등록으로만.
- 인벤토리: read-only 사전점검 — 미산입(**465 불변**, §S13.7 전례).

## §S13.44 고유변동성 spike × 리비전 방향 조건부 틸트 — 사전점검 사전등록 (2026-08-14, 측정 전)

**출처**: 사용자 제안 2026-08-14 — "고유변동성이 평소보다 갑자기 높아졌을
때 리비전/목표주가가 상향이면 good signal, 하향이면 bad signal로 해석해
비중 변화". AskUserQuestion 3결정 확정: ① **사전점검부터 진행**, ② proxy는
**둘 다 측정하되 판정·arm은 `eps_rev_ma_63d` 사전지정**(`tg_mom_63d`는
진단 비액션 — eps_rev FAIL·tg PASS여도 SHELVE, tg 재도전은 새 사전등록),
③ PASS 시 arm은 **대칭 틸트**(good-vol 부스트 + bad-vol 페널티) 사전약정.

**이력 제약(사전 기록)**:
- **§S13.13 `ix_rev_vol`(eps_rev_ma_63d × idio_vol_63d 레벨 z×z 곱) 피처
  arm E1 FAIL(−0.081)·§S13.14 상호작용 피처 축 종결** — 본 후보와 재료가
  같다. 차별점 2가지를 사전 명시: (a) 레벨이 아닌 **자기 역사 대비
  spike**(시계열 조건), (b) 피처가 아닌 **틸트 계층**(§S13.12 전달률 ~9%
  상한 우회 — 유일한 최근 production 성공 §S13.31/32와 동일 계층). 이
  차별이 실증되지 않으면(P2 잔차 FAIL) §S13.13의 재확인으로 종결한다.
- §S13.36 전례: raw IC가 좋아도 최종 score에 기표현이면 잔차 t가 죽는다
  — P2가 본 점검의 핵심 관문.
- §S13.29: 북의 vol 틸트는 구조적(제거 불가). 본 후보는 vol 노출 제거가
  아니라 **방향 조건부 해석**이므로 §S13.27(vol 캡 ΔIR −0.62)과 축이 다름.

**정의 (사전등록 고정, 스윕 금지)**:
- **idio_vol_21d**: `src/features/price.py` 관용 미러 — mkt = 유니버스
  EW 평균 수익률, β = rolling 63BD cov/var, resid = r − β·mkt,
  idio_vol_21d = resid rolling 21BD std × √252. (production
  `idio_vol_63d`와 동일 구조, 창만 21BD — "갑자기"의 포착)
- **spike_z**: idio_vol_21d의 자기 역사 rolling 252BD z(min 126).
  **spike 상태 = spike_z ≥ +1.0**.
- **리비전 방향**: 판정 proxy = harvest 패널 `eps_rev_ma_63d` 부호(>0
  상향 / <0 하향 / 0·NaN 제외). 진단 proxy = `tg_mom_63d` 동일 규칙.
- **표본**: 현 production 등가 harvest
  `outputs/arm_s13_41a_optvol_cov/backtest_result.pkl`(08-12 빈티지,
  기준선 1.6623 산출물)의 리밸런싱 96시점(21BD 간격 = 비중첩). 신호는
  당일 패널·score, 전진 수익률은 §S13.30 `_fwd_return`(t+1..t+21,
  `returns_masked`). 수익률 원천이 masked USD 패널임을 실행 로그로 확인.

**게이트 (판정은 eps_rev proxy·리밸런싱 시점으로만)**:
- **P0 (커버리지, §S13.39 재발 방지)**: 리밸런싱당 평균 spike 종목
  ≥ 10 AND 평균 spike∩상향 ≥ 5 AND 평균 spike∩하향 ≥ 5.
- **P1 (조건부 스프레드)**: spike∩상향 vs spike∩하향의 전진 21BD 평균
  수익률 차(리밸런싱별 스프레드 시계열) — mean > 0 AND t ≥ 2.0 AND
  3분할 서브기간 양(+) ≥ 2/3 (§S13.36 기준).
- **P2 (잔차 직교성 — 핵심)**: 종목 신호 s = spike × sign(rev) ∈
  {−1,0,+1}(scored 전 종목), 최종 score winsor-z 대비 **partial
  Spearman**(s·전진 21BD 수익 양측을 score 랭크에 잔차화한 상관) —
  mean > 0 AND t ≥ 2.0 AND 3분할 양(+) ≥ 2/3. FAIL이면 "모델이 이미
  표현 중"으로 §S13.13 재확인. *(측정 전 정정 2026-08-14: 원안은
  §S13.36 단측 잔차화였으나, 희소 ±1/0 신호에서는 zero-block 잔차가
  −b·z_score로 score 단조가 되어 가짜 rank 상관이 생김을 단위테스트로
  확인 — 실데이터 실행 전에 양측 잔차화로 교체. dense 신호였던
  §S13.36에는 소급 영향 없음.)*
- **종합**: P0∧P1∧P2 전부 PASS → arm **별도 사전등록** 진행. 하나라도
  FAIL → **SHELVE**(구현·인벤토리 없음).

**PASS 시 arm 사전약정(지금 고정 — 결과 본 뒤 변경 금지)**: §S13.31 틸트
관용 — score' = score + **λ·sd(score)·s**, s는 위 정의(대칭 ±1), proxy는
eps_rev, **λ = 0.25 단일**(스윕 금지), E1 = 기존 IR 바(ΔIR > +0.36 &
서브기간 부호 일관) + 캐릭터 보존(§2.5) + DSR 해킷.

**절차/검증 의무**: `scripts/precheck_s13_44_volspike_rev.py`(read-only,
단일 foreground, 백테스트 없음) + 단위테스트(spike 산출·그룹 스프레드·
희소 신호 매핑, fixture 없는 plain 함수). 실행 선행 검증: 패널 컬럼
존재(`eps_rev_ma_63d`·`tg_mom_63d`), predictions 존재, **워크북 mtime
보고**(08-12 빈티지 기대 — §S13.43과 동일 대조). 진단 출력(비액션):
spike 점유율, 그룹별 전진 수익 절대수준, tg proxy 동일 지표, raw IC.
인벤토리: read-only 사전점검 미산입(**465 불변**). 산출물
`outputs/s13_44_volspike_rev_precheck/summary.json`.

**사전 관측 기록**: 모델은 재료(idio_vol·rev 5종·tg_mom)를 전부
보유하고 트리 분기로 상호작용을 내부 형성할 수 있어 **P2가 기본
기대치상 가장 죽기 쉬운 관문**이다(§S13.36에서 raw의 ~70% 기표현
전례). P1은 spike 표본이 얇으면 t가 약할 수 있으나 P0이 먼저 그것을
걸러낸다. SHELVE가 기본 기대치.

### §S13.44 사전점검 실측 (2026-08-14) — P1·P2 FAIL → SHELVE, 방향성은 실재하나 모델이 기표현

실행: `scripts/precheck_s13_44_volspike_rev.py`(단위테스트 4건 PASS),
산출물 `outputs/s13_44_volspike_rev_precheck/summary.json`. 워크북
**08-12 빈티지 일치 확인**(04:45:41 UTC). 표본: 리밸런싱 96시점
(2018-11-27~2026-07-21), 전 종목-일 spike 점유율 18.6%.

- **P0 PASS**: 리밸런싱당 평균 spike 33.9종목, spike∩상향 17.2 /
  spike∩하향 16.8 — 발화는 충분(§S13.39형 희소 발화 아님).
- **P1 FAIL(근소)**: eps_rev 스프레드 **평균 +1.00%p/21BD, 3분할 3/3
  양(+)** — 가설 방향은 전 구간 일관 실재. 그러나 **t +1.85 < 2.0**.
- **P2 FAIL(결정적)**: partial Spearman 잔차 IC 평균 +0.0088,
  **t +0.93** — 최종 score에 직교화하면 증분이 사라진다. spike×rev
  정보는 **모델이 이미 대부분 표현 중**(§S13.36 기표현 전례 재확인,
  §S13.13 `ix_rev_vol` 재료 소진과 정합).
- tg_mom 진단(비액션): 스프레드 +0.18%p, t +0.19 — 목표주가 방향은
  조건부 정보가 사실상 없음. proxy 사전지정(eps_rev)이 옳았고, tg
  재도전 근거도 없음.
- **종합**: 사전등록 종합식(P0∧P1∧P2)에 따라 **SHELVE — arm 미구현,
  λ 틸트 사전약정 소멸**. 해석: "vol spike의 방향 해석"이라는 경제학
  자체는 3/3 서브기간 양의 스프레드로 지지되나, 그 정보는 이미
  eps_rev·idio_vol 피처 경유로 score에 들어가 있어 틸트 계층에서 더
...
*(사후 규명으로 E1 측정이 FX 빈티지 포크 오염 판정 — 아래 참조.
검증자 판정 중 구현·코드 정합·불채택 이행 항목은 유효 유지.)*

### §S13.46 사후 규명 (2026-08-20 오후) — FX 제2 빈티지 축 발견, E1 오염 → 재판정

**발단**: §S13.47 착수 전 점검에서 11:30 정기 배치의 production
재실행이 IR 1.6287116 → **1.6292233**으로 비트 재현 실패(동일 워크북
mtime 08-19 13:49, 동일 명령·코드로 가정했었음). §S13.47 사전등록의
중단 조항 발동 — 규명 결과:

1. **통제 실험**: 동일 코드로 production 1회 추가 재실행(임시 out_dir,
   인벤토리 비산입 재현 점검) → 오늘 배치와 **전 지표 비트 동일**
   (IR/TE/turnover/beta/avg_ic/MaxDD/active_share 17자리 일치).
   **파이프라인은 프로세스 간 완전 결정적** — 재실행 잡음 가설 기각,
   §S13.46 배선의 OFF-parity 위반 가설도 기각(src 차이는 §S13.46
   3파일뿐인데 그 코드로 두 런이 비트 일치).
2. **원인 = 제2 데이터 입력**: 지표 델타 분해에서 **벤치마크 수익률
   자체가 +2.4e-4 이동**(annual Δ2.6e-4 vs active Δ1.7e-5) → 코드가
   아니라 데이터. `fx_source_path` = **`Data/Index.xlsx`(CHF/GBP/DKK
   FX 유일 원천, §S13.34)의 mtime이 08-20 11:33** — 어제 15:13 런
   이후 리프레시됨. 주 워크북만 감시하던 빈티지 체크의 사각.
3. **§S13.46 E1도 오염 확인**: 세 런의 벤치마크 연수익이 전부 상이 —
   base(08-19 15:13) 0.2061839 / arm(08-20 10:56) **0.2058426** /
   배치(08-20 11:56) 0.2064226. 즉 Index.xlsx는 오늘 오전 중 **2회
   이상** 변경(10:56 이전 1회 + 11:33)됐고, §S13.46의 base와 arm은
   **서로 다른 FX 상태**에서 측정됨. 기록된 ΔIR −0.0094는 arm 효과와
   FX 델타의 혼합 — **E1 측정 무효, 재판정 필요**. (avg_ic 비트
   동일은 Spearman의 순위 불변성 때문에 FX 미세 변화를 탐지하지
   못함 — 알파 불변 증명으로서는 여전히 유효.)

**운영 규칙 신설 (이후 모든 arm 비교에 적용)**: 유효 데이터 빈티지 =
**(ai_signal_data.xlsx mtime, Index.xlsx mtime) 쌍**. 모든 비교 런의
전후에 두 mtime을 기록·불변 확인한다. 두 파일 중 하나라도 비교쌍
사이에서 변하면 그 비교는 무효.

**재판정 계획(사전 기록)**: §S13.46 arm을 현 데이터 상태(워크북
08-19 13:49 + Index 08-20 11:33)에서 재실행(동일 사전등록 config,
빈티지 재인증 — 인벤토리 비산입 전례) → 같은 상태에서 산출된 08-20
배치 production(IR 1.6292233)과 A/B. 판정 기준은 §S13.46 사전등록
E1 그대로.

**재판정 결과 (2026-08-20 14:02 재실행, 빈티지 쌍 전후 불변 확인)**:
base 1.6292 → arm **1.6199, ΔIR −0.0093**, 3분할 +0.0748/+0.0110/
−0.1046(부호 비일관), avg_ic 비트 동일, TE 3.457%→3.359%, turnover
0.752→0.734, fallback 0 — 오염 측정(−0.0094)과 사실상 동일. **E1
FAIL·불채택 확정**(이번에는 깨끗한 동일 빈티지 쌍). FX 오염이 이
비교의 결론을 바꾸지 않았음이 실측으로 확인됨. §S13.46의 해석·종결
서술은 그대로 유효하며, e1_summary.json은 재판정 수치로 갱신됨.
**§S13.46 종결(재확정).**
...
**수정 등록 (2026-08-20 오후, 측정 전)**: 위 중단 조항이 실제 발동 —
§S13.46 사후 규명(위)에서 Index.xlsx FX 리프레시로 인한 빈티지
포크로 판명. 이에 따라 **baseline을 08-20 배치 production
IR 1.6292233**(데이터 빈티지 쌍 = 워크북 08-19 13:49 + Index.xlsx
08-20 11:33, 현행 코드, 비트 결정성 통제 실험으로 재현 인증)으로
교체한다. E1 바·3분할 규칙·퇴화율 병기(baseline 13/33) 등 나머지는
불변. 실행 전후 **두 데이터 파일 mtime 불변 확인**을 중단 조항으로
대체 적용한다. 퇴화율 baseline도 08-20 배치 기준으로 재확인해 병기.

### §S13.47 실측 (2026-08-20) — E1 FAIL(바 미달·부호 비일관), 불채택. 단 full +0.20으로 §S13.41 채택치보다 큼

**실행 기록**: schtasks 연속 작업의 2번째 런(14:02~14:32,
`run_variant.py --variant variants/s13_47_ndcg20.yaml --no-cache`),
**빈티지 쌍 전후 불변 확인**(워크북 08-19 13:49 + Index.xlsx 08-20
11:33 — 신설 운영 규칙 최초 적용). ECOS 단일, fallback 0. 판정
`scripts/eval_s13_47_arm.py` → `outputs/s13_47_ndcg20/e1_summary.json`.

**E1 판정 (primary)**:
| 항목 | S0′(08-20 배치) | arm | Δ |
|---|---:|---:|---:|
| full IR | 1.6292 | **1.8320** | **+0.2028** (바 +0.36 미달) |
| 3분할 ΔIR | — | — | +0.0013 / **−0.0125** / **+0.5637** |
| TE | 3.457% | 3.457% | 0 |
| turnover | 0.752 | 0.776 | +0.024 |
| realized_beta | 1.0418 | 1.0495 | +0.008 |
| MaxDD | −32.29% | −32.14% | +0.14%p |
| avg_ic | 0.02035 | 0.01846 | **−9.3%** |
| **퇴화율(병기 의무)** | 13/33 (39.4%) | **10/33 (30.3%)** | **개선** |

→ **E1 FAIL**(ΔIR +0.2028 < +0.36 AND 3분할 부호 비일관) —
사전등록 규칙대로 **불채택**, production `rank_eval_at [5,10]` 유지.

**해석 (진단, 비액션)**:
- 전체 이득 +0.20은 §S13.41 채택치(+0.124)·§S13.31(+0.046)보다 크나,
  **전부 split3(2024-01~)에 집중(+0.564)** — split1 0, split2 소폭
  음. 최근 레짐(메가캡 분산 장세) 특이적일 가능성과, 유니버스
  100→150→200 확장으로 **북이 쓰는 랭크 심도가 깊어진 후반부에서만
  eval@20이 유효**해졌을 가능성이 병존(후자면 구조적 — 판별 불가,
  사전등록된 추가 분석 없음).
- **§S13.8 용량 규제 상실 위험은 미현실화** — 퇴화율 오히려
  13/33→10/33 개선(조기종료가 argmax@20으로 더 오래 학습).
- **avg_ic −9.3%인데 IR +0.20**: top-단면 Spearman IC와 북 수익의
  탈동조 — §S13.12(IC→IR 전달 단절)의 역방향 사례. objective가
  IC가 아니라 북 사용 구간(랭크 6–50)의 서열 품질을 개선했다는
  §S13.45-D 가설과 정합.
- TE 완전 동일·MaxDD 소폭 개선·turnover +2.4%p — 리스크 캐릭터 보존.
- **사용자 오버라이드 검토 시**: §S13.25(+0.266)·§S13.41(+0.124)
  승격 전례와 규모는 비견되나, **부호 비일관(집중 이득)이 전례들과
  다른 약점**. 채택 고려 시 §2.7 DSR/selection-bias 해킷 선행 의무.

인벤토리: **466 → 467** 기록.

**독립 검증자 종합 판정 (2026-08-20)**: 8개 기준 전건 PASS —
판정 스크립트 재실행 = e1_summary = 로그 표 일치(§S13.46 재판정
포함), variant 단일 교체·production yaml/src 무변경, 인벤토리 467,
빈티지 쌍 증빙(로그 6줄 + 현재 mtime) 정합, production
`rank_eval_at [5, 10]` 유지 확인. **§S13.47 종결** — 오버라이드
검토는 사용자 결정 대기(검토 시 DSR 해킷 선행).

### §S13.47 DSR/selection-bias 해킷 (2026-08-20, 사용자 오버라이드 검토 승인에 따름)

`run_selection_bias.py --auto --label s13_47_ndcg20`, **N = 467**
(인벤토리). 산출: `outputs/reports/selection_bias_report.md`.

| 체크 | 결과 | 판정 |
|---|---|---|
| DSR | observed SR 1.794, DSR 1.605, **p = 0.0543** | FAIL(바 0.05 근소 미달, 0.10 미만 WARN 대역) |
| MinTRL | 필요 0.8y vs 보유 8.0y | SUFFICIENT |
| Grid haircut | 1.231 → adjusted SR **0.563** | PASS |
| Survivorship | late entrants **19** | WARN→FAIL 트리거 |
| Sub-period | 1.387 / 0.957 / 2.924 (3/3 양) | STABLE |
| **종합** | | **FAIL** |

**해석(기록)**:
- 종합 FAIL의 결정 요인은 **survivorship 19종**인데, 이는 PLTR·GEV·
  ARM·SNDK 등 **상장 시점이 늦은 종목들의 PIT 마스킹 설계 자체**
  (§S11.4 이중 마스킹·백필 등록, §S9)를 레거시 체크가 인지하지
  못하는 **알려진 구조적 아티팩트** — 동일 유니버스의 어떤 variant
  (production 포함)에도 똑같이 발생하며 arm 고유의 편향이 아님.
- 실질 신호인 **DSR p = 0.0543**은 승격 전례 §S13.41(p 0.1319)·
  §S13.31(p 0.154)·§S13.25(FAIL)보다 **강한 유의성**(0.05 근소 미달).
- Sub-period(절대 IR 기준)는 3/3 양 STABLE이나, E1의 Δ 기준
  부호 비일관(이득 2024+ 집중)은 별개 사실로 유지.
- 채택/보류는 사용자 결정으로 회부(§8 — flip은 사용자 승인
  승격만 가능). 채택 시: production yaml `rank_eval_at: [20]` flip
  1줄 + 새 기준선 재인증 + 독립 커밋 + 대시보드 배치 반영.

### §S13.47 Production flip (2026-08-20) — 사용자 오버라이드 승격

**결정**: 사용자가 E1 FAIL·DSR 게이트 FAIL을 인지한 상태에서
**승격(오버라이드)** 선택(AskUserQuestion 답변). 근거 요약: DSR
p=0.0543은 §S13.41(0.1319)·§S13.31(0.154)·§S13.25(FAIL) 승격
전례보다 강한 유의성, 퇴화율 13/33→10/33 개선, TE·리스크 캐릭터
보존. 인지된 위험(기록): 이득의 2024+ 집중(E1 3분할 부호 비일관) —
레짐 특이 가능성.

**이행**:
1. `variants/codex_causal_rank_65.yaml`의 `rank_eval_at: [5, 10]` →
   **`[20]`** flip 1줄(+승격 주석). **롤백 = 이 1줄 원복**(비트
   결정성 금일 실증 — 구 기준선 pkl은 git f61364e에 보존).
2. 역사적 arm-parity 테스트 6본에 채택 등록(c07165d 전례):
   acceptance 5본은 `set(delta) - {"rank_eval_at"}`, residual_sleeve는
   제외 튜플에 추가. **전체 스위트 567 PASS.**
3. 새 기준선 재인증: production 재실행(schtasks, --no-cache) — 빈티지
   쌍(워크북 08-19 13:49 + Index 08-20 11:33) 불변 전제 하에 arm
   IR **1.8320의 정확 재현(E0)** 기대. 결과는 아래 추기.
4. 독립 커밋 + push(재인증 후).

**새 유효 기준선(재인증 확정 전 잠정)**: production
`codex_causal_rank_65` = rank_eval_at [20], IR 1.8320 / TE 3.457% /
beta 1.0495 / turnover 0.776 / 퇴화율 10/33 (08-19 워크북 + 08-20
11:33 Index 빈티지). **1.6292(구 [5,10])와 혼용 금지.**

**재인증 결과 (2026-08-20 15:46~16:21)**: production 재실행 + 운영
export 완료, 빈티지 쌍 전후 불변. **E0 정확 재현 PASS** — 9개 지표
(IR 1.8319953039451689 / TE 0.03457474623649792 / annual·active
return / turnover / beta / avg_ic / MaxDD / active_share) 전부 arm
run과 **비트 동일**, 퇴화율 10/33·manifest rank_eval_at [20]·ECOS
fallback 0 확인. **새 유효 기준선 확정: production IR 1.8320.**
승격 이행 완료 — 독립 커밋으로 봉인.

## §S13.48 비-피처 계층 구조 결함 3축 사전점검 — 사전등록 (2026-08-21, 측정 전)

**출처**: 사용자 요청 2026-08-21 — "현재 포트폴리오의 성과를 더욱 개선시킬 수
있는 비선형적 요소를 다시 좀 더 찾아줘"(§S13.45 비선형 라운드의 2차). 15에이전트
워크플로우(지도 6 → 생성 5 → 적대심사 3 → 종합 1)로 후보 28건 생성, 3인 적대심사가
9건 사살, 19건 생존, 상위 6건 랭킹. AskUserQuestion으로 **트랙 A(직교 3축 병렬
사전점검)** + **오버라이드 정책(전례대로 검토)** 확정. 통과 축만 arm을 **별도
사전등록** 후 실행한다(이 점검은 arm이 아님).

**공통 표본·프레임 (고정)**:
- 표본 pkl = `outputs/codex_causal_rank_65/backtest_result.pkl` — **08-21 12:15
  정기 배치 산출**(커밋 `0026117`). production IR **1.8319953039451689** /
  TE 0.03457474623649792 / avg_ic 0.018456457854361133 / beta 1.0494815747554644 /
  turnover 0.7764318065684073 / MaxDD −0.3213831702536806 / 퇴화율 10/33.
- **데이터 빈티지 쌍** = 워크북 `ai_signal_data.xlsx` **08-19 13:49** +
  `Index.xlsx` **08-21 11:16**. Index.xlsx가 08-20 11:33 → 08-21 11:16으로
  갱신되었음에도 **IR이 비트 동일 재현** — 이번 리프레시는 과거 구간 FX를
  변경하지 않았다(§S13.46형 빈티지 포크 **아님**). 실행 전후 두 파일 mtime
  불변 확인을 중단 조항으로 적용한다(§S13.47 신설 규칙 승계).
- 리밸런싱 그리드 = `res.portfolio_weights` 시점 **97개**.
- 전진 수익률 = §S13.30 `_fwd_return` 관용(t+1..t+21, masked USD 패널).
  벤치마크 = `get_benchmark_fn(data, tickers, config=cfg)`.
- 사전비중 복원(공통): 리밸일 t의 **사전(드리프트 후) 비중**
  w⁻_t = `_drift_weights(daily_weights[t_prev], r_t)`,
  t_prev = `daily_weights` 인덱스의 t 직전 영업일, r_t = `returns_masked.loc[t]`
  (결측 0). 드리프트 식은 `backtest.py:1306-1318` 그대로 w·(1+r)/Σ.
  사후비중 w⁺_t = `res.portfolio_weights[t]`. **δ_t = w⁺_t − w⁻_t**,
  turnover_t = Σ|δ_t| (two-way L1, backtest 관용과 동일).
- 판정은 각 축의 **사전지정 primary로만**, 나머지는 진단 비액션.
- 인벤토리: read-only 사전점검 **미산입**(**467 불변**, §S13.7·13.36·13.39 전례).
- 사전약정: PASS 축의 arm은 §S13.45 관용대로 **전부 비-피처 계층**이며,
  랭커 피처 주입 경로는 금지한다.

### 사전 관측 기록 (정직 고지 — A축은 1일 오염됨)

A축은 **설계 단계에서 이미 1개 리밸일이 관측되었다**.
`outputs/operating/expected_rebalance.json`(as_of 2026-08-19,
`is_rebalance_data_as_of`=true, signal_confidence 1.0) 200행 실측:

| 항목 | 실측 |
|---|---:|
| δ가 소수 **7자리까지 동일**한 종목 수 | **175 / 200** (δ = −0.0001401) |
| 그 균일 블록의 two-way L1 점유 | **35.4%** (0.024517 / 0.069289) |
| 블록 내부 μ 범위 / sd | **[−1.108, +5.216] / 0.783** |
| \|δ\| ≥ 0.003 실거래 종목 | **5 / 200** |
| \|δ\| < 0.003 종목 | 195 / 200 |

**따라서 A의 P0(일반성)와 P1의 부수보고(μ 스프레드)는 이 1일 관측에 오염되어
있다.** A의 진짜 사전검정은 **P1의 전진수익 기반 Ĝ**와 **P2(경제성)**뿐이며,
P0는 "1일 관측이 97일로 일반화되는가"의 확인 역할로 격하해 기록한다.
**B·C는 사전 관측 0건**(어떤 수치도 측정되지 않은 상태에서 아래 바가 고정됨).

### §S13.48-A 투영 계량의 알파 맹목성 (균일 클로백)

- **구조 사실(코드 확인 완료)**: `backtest.py:1242-1246`이
  `delta[|delta| < ntb] = 0.0` 후 **재정규화 없이** `candidate = prev + eta·delta`
  를 만들어 Σcandidate ≠ 1이 되고, `portfolio_optimizer.py:543`의
  `cp.Minimize(cp.sum_squares(w − candidate))` + `cp.sum(w) == 1`이 예산 위반을
  **μ가 목적함수에 전혀 없는 균일 절대 이동**으로 복구한다. 노-트레이드 밴드가
  죽인 거래를 투영이 전 종목에 되살린다. 결정로그 전수에서 **μ가 투영 목적함수에
  들어간 arm은 0건**.
- **정의(고정)**: 균일 클로백 군집 = δ_t를 **소수 7자리 반올림**한 최빈값 c_t와
  그 종목 수 m_t(7자리는 08-19 관측에서 solver 노이즈가 9자리에만 있었던 사실을
  반영해 측정 전 고정). μ = `res.predictions.loc[t]`(오버레이·실행lag 반영 후 =
  옵티마이저가 실제로 본 값). fwdex21_i,t = 종목 i의 t+1..t+21 누적수익 −
  동기간 벤치마크 수익.
- **P0 (일반성)**: **S = mean_t(|c_t|·m_t / turnover_t) ≥ 0.20** (97 리밸 전 구간).
  부수보고: S 시계열 분포, m_t 중앙값, c_t 부호 분포. 미달 = 08-19이 특이일 →
  SHELVE.
- **P1 (증분 — 이 축의 본 검정)**:
  **Ĝ = Σ_t |c_t|·(m_t/2)·[mean(fwdex21 | 군집 상위 μ 절반) −
  mean(fwdex21 | 군집 하위 μ 절반)] > 0 AND 시간순 3분할 3/3 양**.
  = 균일 클로백을 μ 순서대로 배분했을 때의 **반사실 이득**. 부호 비일관 또는
  Ĝ ≤ 0 → SHELVE. 부수 필수: 군집 내부 μ sd 중앙값(08-19의 0.783이 일반적인지).
- **P2 (경제성)**: Ĝ의 연율 환산 **≥ +0.10%p/yr**(액티브 수익 기준).
  미달이면 시드 밴드 ±0.19(§S7.A4)에 묻혀 어떤 부호가 나와도 판정 불가 → SHELVE.
- **PASS 시 arm 사전약정(스윕 금지)**: `projection_alpha_weight_kappa: 3.0`
  (default **0.0**). 투영 목적함수를 μ-랭크 가중 최소제곱
  `Minimize(sum_squares(multiply(sqrt(v), w − candidate)))`,
  **v_i = 1 + 3·pct_rank(μ_i) ∈ [1,4]**(μ 비유한은 v=1 → 고-μ일수록 덜 움직임).
  **κ == 0.0일 때 원래 표현식을 그대로 타는 명시적 분기 필수**(가중 이차형은
  canonicalization이 달라 ECOS 최말단 비트가 어긋날 수 있어 OFF-parity가
  구조적으로 보장되지 않음). κ 스윕 금지.
- **E1**: ΔIR > +0.36 vs **1.8320** AND 3분할 부호 일관. **병기 하드 FAIL**:
  ① turnover Δ ≤ **+1%p**(설계 전제가 '거래량 불변'), ② projection solver
  fallback **1건이라도 발생 시 즉시 FAIL**(fallback mode='target'이라 조용히
  target 북으로 떨어짐), ③ avg_ic **비트 동일**(0.018456457854361133) —
  불일치 시 배선 오류로 중단, ④ TE ≤ 3.5%, Pictet active_share ≥ 18%.

### §S13.48-B RMT 벌크 고유값 정제 (Σ 고유공간)

- **구조 사실(코드 확인 완료)**: `portfolio_optimizer.py:146-149`의
  `floor = max(fallback_var*1e-4, 1e-10)`가 **N=200 > T=126**(cov_lookback)에서
  랭크 결핍으로 생긴 다수의 사실상 무위험 고유방향을 만들고, 옵티마이저가 그
  방향에 액티브 리스크를 무료로 적재할 수 있다. §S13.41(대각 값)·§S13.42(대각
  예측)·§S13.46(비대각 스칼라)은 전부 Σ의 **값**만 바꾸고 고유공간을 승계했다 —
  **고유공간 자체를 바꾼 arm은 0건**(`eigen`/`RMT` grep 0).
- **정의(고정)**: Σ_t = `estimate_covariance(hist_returns_t, bm_weights=bm_t,
  config=cfg)`(production 경로 그대로), hist_returns_t = `returns_masked`의 t
  직전 126 영업일. q_t = N_eff,t/126, σ̄²_t = trace(Σ_t)/N_eff,
  **λ₊_t = σ̄²_t(1+√q_t)²**. Σ_RMT = λ ≤ λ₊ 인 고유값 전부를 **그 부분집합
  산술평균**으로 치환, λ > λ₊ 무변경, trace 보존 재스케일, 대칭화.
  a_t = w⁺_t − bm_t(실제 실행된 액티브).
- **P0 (예측력)**: 전진 21BD 실현 액티브 분산 타깃 **excess-QLIKE 개선율 ≥ +5%
  AND 시간순 3분할 부호 일관**(§S13.41 P0 / §S13.42 P0′와 **동일 바** — 자의성 0).
- **P1 (증분·2중)**:
  (i) **바인딩** — baseline Σ_t에서 floor에 닿은(λ ≤ floor·(1+1e-9)) 고유방향들이
  설명하는 aᵀΣa 점유율의 **중앙값 ≥ 10%**.
  (ii) **스칼라-등가 제거** — Σ_RMT와 aᵀΣa가 같아지도록 baseline을 균일 배율한
  Σ_scalar 대비, Σ_RMT로 재-MVO(**24회 균등 샘플**,
  `precheck_s13_41_optvol_cov.py` 관용구) 시 **one-way L1 이동 중앙값 ≥ 0.005**
  (§S13.41 P1 실측 달성치 0.0245). 미달 = §S13.22형 대칭 스케일링 축퇴 → SHELVE.
- **P2 (경제성·캐릭터)**: 24 샘플 재-MVO 북에서 Pictet **active_share ≥ 18%**
  AND ex-ante **TE ≥ 3.0%**. **부수 필수**: 치환된 고유방향 수 / N_eff의 중앙값 —
  **0.95 초과 시 Σ가 대각+평균상관으로 축퇴**해 §S13.46/스칼라 채널과 구분
  불가해지므로 그 사실을 판정에 병기하고 arm 설계 재검토 사유로 기록.
- **PASS 시 arm 사전약정(스윕 금지)**: `cov_rmt_denoise_enabled: true` 1개.
  상수 전부 모듈 고정. 적용 순서는 메가캡 수축·§S13.41 대각 스케일보다 **앞**.
- **E1**: ΔIR > +0.36 AND 3분할 부호 일관. **병기 하드 FAIL**: active_share < 18%
  또는 실현 TE < 3.0% → 즉시 FAIL(§2.5 캐릭터 붕괴). 기타: TE ≤ 3.5%,
  turnover Δ ≤ +2.5%p, avg_ic 비트 동일, solver fallback 0.

### §S13.48-C mega funding_set 이산 재선택 히스테리시스

- **구조 사실(코드 확인 완료)**: `portfolio_optimizer.py:381-392`가 매 리밸
  funding_set을 **처음부터 재선택**(mega_indices = {bm ≥ 0.04}, eligible =
  {μ < 0.0} 엄격부등, μ 오름차순 상위 k=4)하므로, mega 간 **μ 순위 치환만으로**
  멤버십이 뒤집히고 그 종목의 제약이 **최대 −10% UW 허용(`:389`) ↔ w ≥ bm
  강제 핀(`:392` max_uw_per=0.0)** 사이를 점프한다. bm 4~8% 종목의 수백 bp
  강제거래가 **알파 크기 변화 없이** 발생한다. §S13.27-A(캡 **값**을 연속함수로
  교체, −0.0998로 사망)와 정반대 — 값은 불변, 멤버십 **시점**만 늦춘다.
- **P0 (실재성)**: 인접 리밸 간 funding_set 멤버십 flip이 1건 이상인 리밸 비율
  **f ≥ 0.25**.
- **P1 (증분·2중)**:
  (i) **프록시 타당성** — 실제 funding_set ⊆ {i : w⁻_i < bm_i − 0.005}
  **일치율 ≥ 95%**(하드 게이트: w⁻는 드리프트 북이라 잡음·투영 fallback 오탐 가능).
  (ii) flip 종목의 실측 |δ| 중앙값 / 비-flip mega |δ| 중앙값 **≥ 2.0배**
  (그 회전이 통상 리밸런싱으로 이미 설명되지 않는 증분임을 증명).
- **P2 (경제성)**: flip 종목 귀속 Σ|δ| / 전체 two-way L1 **≥ 0.05**.
- **PASS 시 arm 사전약정(스윕 금지)**:
  `mega_cap_funding_hysteresis_enabled: true`, `uw_tol` **0.005 모듈 고정**.
  유임 규칙 = μ_i < 0 이고 bm_i − w⁻_i > 0.005 인 mega를 μ 오름차순으로 **먼저**
  슬롯 배정, 잔여 슬롯만 나머지 eligible의 argmin으로 충원.
  k=4·bm_thr 0.04·score_max 0.0 전부 불변. tol 스윕 금지.
- **E1**: ΔIR > +0.36 AND 3분할 부호 일관. **병기**: avg_ic **비트 동일**
  (불일치 시 배선 오류로 즉시 중단), turnover Δ ≤ 0 기대, TE ≤ 3.5%.
  **사후 귀속 의무**: 유임 규칙이 μ가 더 나쁜 mega를 붙잡는 알파 손실 정량화
  (유임 종목 전진 21BD 초과수익 vs 밀려난 후보).

### 오버라이드 정책 — 사용자 사전 비준 (2026-08-21, 측정 전)

E1 형식 바 +0.36은 **5년 표본에서 도달 전례 0건**이다(§S13.25-A +0.266,
§S13.47 +0.203, §S13.41 +0.124 — 전부 형식 FAIL 후 오버라이드 승격).
사용자가 **측정 전에** 다음을 비준했다: arm이 (a) ΔIR > 0, (b) 시간순 3분할
부호 일관, (c) 리스크 캐릭터 보존(TE ≤ 3.5%, active_share ≥ 18%, solver
fallback 0), (d) DSR p가 승격 전례(§S13.31 0.154 / §S13.41 0.1319 /
§S13.47 0.0543) 대비 강함 — 을 **모두** 충족하면 형식 FAIL이어도 **승격 후보로
회부**하고 최종 결정은 사용자가 한다. **이 방침은 측정 전 등록이므로 사후 바
조정이 아니다**(§8 flip은 여전히 사용자 승인 승격만 가능).

### 절차/검증 의무 (공통)

스크립트 3본 `scripts/precheck_s13_48{a,b,c}_*.py`(read-only, **백테스트 없음**)
+ 각 단위테스트(plain 함수, fixture 없음, `np.allclose(..., atol=1e-6)` 관용).
**실행 선행 검증**(불일치 시 진행 중단·보고만): 빈티지 쌍 mtime(워크북
08-19 13:49 / Index 08-21 11:16), pkl last `daily_weights` == 2026-08-19,
`returns_masked` 존재, `predictions`·`portfolio_weights`·`daily_weights` 존재.
산출물 `outputs/s13_48{a,b,c}_*/summary.json`. 구현은 **독립 에이전트 3본 병렬**
(스크립트+단위테스트만 — 사전점검 본실행 금지) → **메인이 단일 foreground로 순차
실행**(§1 좀비 hang 규칙) → 독립 검증자 종합 판정.

**사전 기대치 기록**: A는 위 사전 관측 기록대로 **1일 오염**(P0는 확인용,
본 검정은 P1 Ĝ). B는 q ≈ 1.59 → λ₊ ≈ 5.1σ̄²이라 P2의 치환 비율이 1.0에 근접할
위험이 **사전 인지**됨(그 경우 대각+평균상관 축퇴로 판정에 병기). C는 21BD
간격에서 mega μ 순위가 이미 안정하면 P0에서 즉사(20분에 판별되는 것이 유일한 강점).
**SHELVE가 기본 기대치** — 3축 각 P1 통과 확률 30~40% 추정, 3축 전부 SHELVE가
최빈 결과.

### §S13.48 사전점검 실측 (2026-08-21) — A·B·C **전부 SHELVE**

실행: `scripts/precheck_s13_48{a,b,c}_*.py` 각 **단일 프로세스 순차**(A 412s →
C 400s → B 344s, 동시 실행 없음). 단위테스트 16건(A 5 / B 6 / C 5) 전건 PASS —
메인이 직접 재실행 확인. 산출물 `outputs/s13_48{a,b,c}_*/summary.json`.
**빈티지 3중 가드 전 축 통과**: pkl 2026-08-21 12:15:13 / 워크북 2026-08-19
13:49:14 / Index.xlsx 2026-08-21 11:16:45 / last `daily_weights` 2026-08-19 —
세 스크립트 로그에 동일 출력. 실행 전후 두 데이터 파일 mtime 불변.
인벤토리: read-only 사전점검 **미산입**(**467 불변**).

#### A (투영 계량의 알파 맹목성) — **SHELVE** (P0 FAIL / P1 PASS / P2 FAIL)

| 게이트 | 실측 | 바 | 판정 |
|---|---:|---:|---|
| P0 일반성 S | **0.1523** | ≥ 0.20 | **FAIL** |
| P1 반사실 이득 Ĝ | **+0.004157**, 3분할 [+0.000852, +0.001478, +0.001827] | >0 & 3/3 양 | **PASS** |
| P2 연율 환산 | **+0.000538 (+0.054%p/yr)** | ≥ +0.0010 | **FAIL** |

n_used 81 / skipped 16(`split_empty` 14, `no_prev_daily` 1, `no_fwd_window` 1),
years 7.73.

- **메커니즘은 실재·부호 일관**: P1이 깨끗이 통과했고 3분할이 **단조 증가**
  (0.00085 → 0.00148 → 0.00183). 균일 클로백을 μ 순서로 배분하면 97 리밸
  전 구간에서 양의 초과수익이 났다(`g_mean` = +1.14%p / 21BD 단면 스프레드).
  즉 "L2 투영이 μ 정보를 버린다"는 구조 진단 자체는 **참으로 확인**됐다.
- **죽은 이유는 규모**: 08-19의 35.4%는 **상위 꼬리**였다 — 전 구간 S 평균
  0.1523 / 중앙값 0.1304 / 최대 0.4938 / 바 이상 비율 **28.4%**뿐.
  `|c_t|` 중앙값 4.29e-05는 08-19(1.401e-04)의 **약 1/3**. 연율 이득
  +0.054%p는 시드 밴드 ±0.19 IR(§S7.A4)에 완전히 묻힌다. **P2 바는 사후
  조정하지 않는다.**
- **사전 오염 고지가 실효를 발휘**: 사전등록이 A의 1일 관측 오염을 명시하고
  P0를 "1일 관측의 97일 일반화 확인"으로 격하해 둔 덕분에, 1일 관측이 3배
  과대추정이었다는 사실이 게이트에서 잡혔다.
- **워크플로우의 사망 예측은 빗나감**: "군집 내부 μ 스프레드가 0에 수렴"이
  최다 사망 지점으로 예측됐으나 `mu_sd` 중앙값 **0.948**로 오히려 08-19
  (0.783)보다 넓었다. 사망 원인은 μ 스프레드가 아니라 **클로백 총량**이었다.
- 진단(비액션): c_t 부호 negative 41 / zero 4 / positive 36, m_t 중앙값 159
  (min 2, max 186), turnover_t 평균 0.0628.

#### B (RMT 벌크 고유값 정제) — **SHELVE** (P0 FAIL / P1(i) FAIL / P1(ii) PASS / P2 PASS)

| 게이트 | 실측 | 바 | 판정 |
|---|---:|---:|---|
| P0 excess-QLIKE 개선 | **−51.4%**, 3분할 [−0.4545, −0.2440, −0.8374] | ≥ +5% & 부호 일관 | **FAIL** |
| P1(i) floor 방향 분산 점유 중앙값 | **0.0%** | ≥ 10% | **FAIL** |
| P1(ii) 재-MVO one-way L1 중앙값 | 0.0126 (n=23) | ≥ 0.005 | PASS |
| P2 active_share / ex-ante TE 중앙값 | 22.5% / 3.89% | ≥18% / ≥3.0% | PASS |

- **가설의 전제가 정면 반증됨**: P1(i)가 **정확히 0.0%** — eigen floor에 닿은
  고유방향들이 실행된 액티브의 ex-ante 분산을 **한 톨도 설명하지 않는다**.
  "N=200 > T=126의 랭크 결핍 방향에 옵티마이저가 리스크를 무료로 적재한다"는
  가설은 **거짓**이다. 제약 집합(active share L1 ≤ 0.45, max_active_per_stock
  0.04, 섹터, mega-cap)이 그 방향에 도달하기 전에 먼저 바인딩하는 것으로 보인다
  (귀속은 사전등록 밖 — 비액션 추정으로만 기록).
- **RMT는 예측을 오히려 크게 악화**: P0 −51.4%, 3분할 전부 음. 이는 축퇴 경고와
  정합한다 — **`replaced_share` 중앙값 0.970**(사전등록에서 0.95 초과 시 병기
  의무로 등록해 둔 조건이 실제 발동). q ≈ 1.59에서 λ₊ ≈ 5.1σ̄²이라 스펙트럼의
  97%가 평균으로 치환되어 Σ가 사실상 **대각+평균상관**으로 무너지고, 실재
  팩터 구조까지 지워진다. §S13.46/스칼라 채널과 구분 불가.
- P1(ii)·P2가 통과한 것은 "북을 움직이기는 하고 캐릭터는 보존한다"는 뜻일 뿐,
  P0·P1(i)가 죽은 이상 **비액션**이다.
- **결론**: Σ 고유공간 축은 이 형태(MP 경계 벌크 평균화)로는 **종결**.
  단 P1(i)=0.0%는 별개의 구조 지식 — **옵티마이저는 저위험 고유방향을 이미
  쓰지 않는다**(무료 리스크 적재 우려는 이 유니버스·제약에서 기우).

#### C (mega funding_set 히스테리시스) — **SHELVE** + **가설 메커니즘 반증(§9 사유)**

| 게이트 | 실측 | 바 | 판정 |
|---|---:|---:|---|
| P0 flip 발생 리밸 비율 f | **0.695** (66/95) | ≥ 0.25 | PASS |
| P1(i) 프록시 타당성 | **0.324** (77/238) | ≥ 0.95 | **FAIL** |
| P1(ii) flip \|δ\| 배율 | 2.014 (0.000158 / 0.0000786) | ≥ 2.0 | PASS |
| P2 flip 귀속 회전 점유 | **0.0394** (0.2433 / 6.1767) | ≥ 0.05 | **FAIL** |

cfg 실측 파라미터: `mega_cap_bm_threshold` 0.04 / `mega_cap_funding_k` **4** /
`mega_cap_funding_score_max` 0.0 / protection·funding_mode 둘 다 True.

- **§9 발동 — 사전등록한 메커니즘이 실제와 다르다**: 진단상
  **mega 종목 수 중앙값 4.0**(min 2, max 5)인데 **funding_k도 4**다. 따라서
  `eligible.sort(...)[:k]` **절단이 사실상 발동하지 않으며**, funding_set ≡
  {mega 중 μ < 0인 종목}이 된다(funding_set 크기 중앙값 3). 사전등록이 가정한
  "μ **순위 치환**으로 멤버십이 뒤집힌다"는 경로는 **존재하지 않는다**.
- **실제 flip 구동원은 μ의 절대 임계 0.0 가로지르기**다 — 4종뿐인 mega 풀에서
  리밸당 중앙 1건(히스토그램 0건 29 / 1건 46 / 2건 20)이 부호를 넘나들며 제약이
  '−10% UW 허용 ↔ w ≥ bm 강제 핀' 사이를 점프한다. 이는 워크플로우가 **별개
  후보(score-gate 절대 0.0 → 단면 분위수, 슬레이트 5위)**로 분류한 채널이다.
- 따라서 사전약정한 히스테리시스 arm(슬롯 배정 순서 규칙)은 **표적을 잘못 겨눈
  설계**였고, P1(i)의 하드 게이트가 이를 정확히 잡았다. P2는 mega가 4종뿐이라
  상금 자체가 전체 회전의 3.9%로 작음을 보인다.
- **후속 리드(미회수)**: score-gate의 절대 0.0 임계 → 단면 분위수 정합 후보는
  **C의 진단으로 사전확률이 올라갔다**(μ 부호 가로지르기가 70% 리밸에서
  실재함이 mega 부분집합에서 실증). 단 mega 4종이 아니라 **200종 전체**를
  대상으로 하므로 P2(상금 규모)가 완전히 다시 계산되어야 한다 —
  **새 사전등록 필수**, 이 절의 수치를 근거로 바를 정하지 말 것(선택 편향).

#### §S13.48 종합

- **3축 전부 SHELVE** — 사전등록의 최빈 기대치("3축 전부 SHELVE")와 일치.
  게이트는 설계대로 작동했다: A는 규모, B는 전제, C는 메커니즘에서 각각 죽었다.
- **arm 0건 실행 → 인벤토리 467 불변**, production 무변경(`variants/
  codex_causal_rank_65.yaml` 무편집), src 무편집.
- 사용자가 §S13.48에서 **측정 전 비준한 오버라이드 정책**은 이번에 발동 대상이
  없다(arm 미실행). 정책 등록 자체는 유효하게 존치한다.
- **구조 지식 3건 획득(비액션)**: ① L2 투영은 μ 정보를 실제로 버리며 그 반사실
  이득은 부호 일관(단 연 0.054%p로 환전 불가), ② 옵티마이저는 저위험
  고유방향을 이미 쓰지 않는다(무료 리스크 적재는 이 제약 집합에서 기우),
  ③ mega funding_set의 top-k 절단은 inert이고 실제 churn은 μ 부호 게이트가
  만든다.


### §S13.48 독립 검증 판정 + 기록 정정 (2026-08-21, 커밋 2ef9884 이후)

독립 검증자 9기준 판정: **8/9 PASS, V9 FAIL → 종합 FAIL**. 단 결함은 **C의 §9
서사 근거력과 문언**에 있고, **게이트 산술·A·B·C의 SHELVE 결론·production
무변경은 전부 영향 없음**(V1·V2·V4·V5·V6·V7·V8 전건 PASS). 메인이 지적 3건을
전부 코드·데이터로 재확인했고 사실로 확정되어 아래와 같이 정정한다.

**정정 1 — B의 "정확히 0.0%"는 과장(문언 오류)**
위 §S13.48-B 서술의 "P1(i)가 **정확히 0.0%**", "한 톨도 설명하지 않는다"는
거짓이다. 실측은 `median_floor_share` = **3.8928e-06**(mean 4.3375e-06),
즉 **0.00039%**이다. 표의 "0.0%"는 정당한 반올림 표기이나 본문의 "정확히"는
성립하지 않는다. **정정 후 표현**: "floor 방향의 액티브 분산 점유는 중앙값
3.89e-06으로 바(0.10)보다 **4.5자릿수 작다** — 실질적으로 0이나 정확히 0은 아니다."
판정(FAIL)과 결론(무료 리스크 적재 우려는 기우)은 불변.

**정정 2 — B는 production과 다른 수익률 패널로 측정됨 (SPEC-CODE MISMATCH, 기록 누락 보정)**
`outputs/s13_48b_rmt_denoise/summary.json`의 `notes`가 이미 기록한 사실이
로그 본문에 전이되지 않았다. 사실관계:
- 사전등록 §S13.48-B는 Σ 입력을 `data.returns_masked`로 지정했고 구현은 이를
  축자 이행했다.
- **production 경로는 `data.raw_returns`를 쓴다** — `backtest.py:1989-1991`
  (`risk_returns = getattr(data, "raw_returns", None)`) → `:1432`
  (`risk_source = risk_returns if risk_returns is not None else returns`).
- 따라서 본 사전점검의 Σ_base는 **production Σ와 동일하지 않다**.
  `estimate_covariance`는 `recent.isna().any().any()`일 때만
  `_pairwise_covariance`(= eigen floor 보유 경로)로 가므로, 패널이 다르면
  floor 경로 진입 자체가 달라질 수 있다. 본 실행의 진입 횟수는
  `pairwise_path_dates` = **87/95**, `median_floor_ref` = 2.8279e-08.
- **결론에 대한 영향 평가**: P1(i) 3.89e-06 vs 바 0.10(4.5자릿수), P0 −51.4%
  vs 바 +5% — 두 게이트 모두 압도적 차이로 실패하므로 패널 차이로 부호가
  뒤집힐 여지는 없다고 판단한다. 단 **"production Σ에서 측정했다"고 주장할 수
  없으며**, 그 주장은 이 절로 철회한다. 재도전 시 사전등록은 반드시
  `raw_returns`를 지정해야 한다.

**정정 3 — C의 §9 반증 서사는 과장, 아래로 낮춘다**
(a) "`eligible[:k]` 절단이 inert"는 **전 구간에서 증명되지 않았다**.
`by_date.csv` 재집계(메인 직접 확인): n_mega 분포 = {2:4일, 3:28일, 4:37일,
**5:27일**}. 절단이 확실히 inert(= `funding_set_size < min(k, n_mega)`)인 날은
**72일**, `n_mega ≤ 4`라 전원 수용되는 날을 더하면 **88/96일에서 inert 증명**.
그러나 **`n_mega=5 ∧ funding_set_size=4`인 8일**(2024-06-17, 2025-04-07,
2025-09-01, 2025-10-29, 2025-11-27, 2025-12-26, 2026-01-26, 2026-05-22)은
eligible 크기(μ<0 mega 수)를 summary/by_date에 남기지 않아 **절단 바인딩
여부 판정 불가**다. → 정정 표현: "절단은 **88/96일(91.7%)에서 inert로 증명**,
8일은 미기록으로 판정 불가."
(b) "실제 구동원은 μ의 절대 0.0 가로지르기"도 **단독 원인이 아니다**.
exit 44건 중 **17건이 n_mega 감소일**에 발생 — 이는 bm이 0.04 아래로 내려가
**mega 집합 자체가 축소된 별개 채널**이다. 또 `entered_mu`가 전건 음수
(max −0.0037)인 것은 `s < score_max(0.0)` 필터가 강제하는 **동어반복**이라
메커니즘 증거가 아니다. → 정정 표현: "flip의 상당수는 μ 부호 가로지르기이나
**최소 17/44 exit은 mega 집합 축소(bm<0.04) 채널**이며, 두 채널은 본 산출물로
분리되지 않는다."
(c) P1(i) 0.324 미달을 "설계가 표적을 잘못 겨눔"으로 해석했으나, **프록시
(w⁻ 기반) 자체의 부정확** 가능성과 구분되지 않는다(사전등록도 "드리프트 북
잡음·투영 fallback 오탐 가능"을 인정했다). 두 설명은 배제되지 않은 채 병존한다.

**판정에 대한 영향**: 위 정정 3건은 전부 **서술의 강도**를 낮출 뿐,
A·B·C의 게이트 산술과 **SHELVE 판정, arm 미실행, production 무변경,
인벤토리 467 불변**은 그대로다. §S13.48은 정정된 상태로 종결한다.

**프로세스 개선 채택 (검증자 ④(i) 수용)**: 사전등록 절과 실측 절이 동일
미커밋 diff에 함께 있으면 **바가 측정 전에 고정됐다는 사실이 git으로 독립
입증되지 않는다**(이번엔 스크립트 mtime 체인 15:08/15:09/15:12 < 산출물
15:20/15:27/15:33로 대체 입증됐다). **앞으로 사전등록은 측정 전에 단독
커밋한다** — §S13.49부터 즉시 적용.

## §S13.49 score-gate 바인딩 사전점검 (절대 0.0 → 단면 분위수 후보의 전제 검정) — 사전등록 (2026-08-21, 측정 전)

**출처**: §S13.48-C의 §9 반증이 승격시킨 후보(워크플로우 슬레이트 5위 G5-3) +
사용자 승인 2026-08-21 "G5-3 score-gate 분위수 진행". **성능 arm이 아니라
전제(바인딩) 검정**이다 — §S13.7(그룹별 z-score, η² 0.011로 arm 미실행)·
§S13.36(share_turnover 틸트, 잔차 IC t 1.04로 arm 미실행) 전례를 따른다.

**구조 사실(코드 확인 완료)**: `src/portfolio_optimizer.py:413-421` —
`enforce_score_gated_ow`가 True일 때 `score_i <= score_threshold_for_ow`(=**0.0**,
**절대 레벨**)인 전 종목에 `w_i <= bm_i` 제약을 건다. μ는 오버레이·실행lag 반영
후 값이므로 **게이트 대상 종목 수가 μ 분포의 위치 이동에 따라 매일 흔들린다**.
동일 제약이 `optimize_portfolio`(MVO)와 `project_portfolio_weights`(투영)
**양쪽**에 들어간다.

### 사전 관측 기록 (정직 고지 — 1일 오염, 사전확률을 크게 낮춤)

설계 전에 `outputs/operating/expected_rebalance.json`(as_of **2026-08-19**,
실제 리밸일) 200행을 관측했다:
...
| 유효 μ | 200 / 200 |
| 게이트 대상(μ ≤ 0) | **132 (66.0%)** |
| 게이트 대상 중 경계(\|w−bm\| ≤ 1e-6)에 붙은 종목 | **0** |
| `bm − target` 최소 / 중앙값 | **9.0e-05** / 6.6e-04 |
| 실제 OW 종목(μ>0 & w>bm) | **12** |

**즉 이 날짜에서 게이트는 어느 종목에도 바인딩하지 않았다.** 볼록 문제에서
slack 제약은 해에 영향이 0이므로, 임계를 분위수로 바꿔도 **구조적 no-op**일
가능성이 크다. 이 관측은 아래 G0의 바를 정하는 데 쓰였으므로 **G0는 1일
오염**이며, 축의 진짜 판정은 **G1(반사실 재-MVO)** 이다. G1·G2는 사전 관측 0.

**단, 1일 관측만으로 종결하지 않는 이유**: (a) `expected_rebalance.json`의
`target`은 **투영 단계 산출물**이라 MVO 단계에서의 바인딩 여부를 말해주지 않는다,
(b) 코드 주석(`:415-418`)이 "`<`였을 때 score==0.0 종목을 MVO가 분산화 항으로
OW할 수 있었다"고 명시 — 즉 바인딩 사례가 과거에 실재했다, (c) 1일은 §S13.48-A
에서 3배 과대추정으로 이미 데인 표본 크기다.

### 공통 표본·프레임 (고정)

§S13.48과 동일: pkl `outputs/codex_causal_rank_65/backtest_result.pkl`
(08-21 12:15 배치, IR 1.8319953039451689), 빈티지 쌍 = 워크북 08-19 13:49 +
Index.xlsx 08-21 11:16, 리밸 그리드 97, `data.returns_masked`, 실행 전후 mtime
불변 확인 의무. 인벤토리: read-only 사전점검 **미산입(467 불변)**.

### 게이트 (측정 전 고정 — 셋 **전부** PASS여야 arm 설계 자격)

- **G0 (최종 북 바인딩)**: 97 리밸 각각에서 게이트 대상 종목
  (μ ≤ 0, **mega(bm ≥ 0.04)와 invalid_alpha 종목은 제외** — 그쪽은 다른 제약이
  w를 bm에 고정하므로 교락) 중 `|w⁺_i − bm_i| ≤ 1e-6` 인 비율을 계산.
  **그 비율의 중앙값 ≥ 0.05**. 부수 필수 보고: `bm − w⁺` 의 분위수(min/p10/median),
  게이트 대상 수 g_t·OW 종목 수의 시계열 분포.
- **G1 (반사실 바인딩 — 이 축의 결정적 검정)**: 리밸 그리드에서
  `np.linspace(0, 96, 24, dtype=int)` 24개 균등 샘플. 각 샘플에서
  `optimize_portfolio`를 **동일 인자로 두 번** 호출하되 `enforce_score_gated_ow`만
  True/False로 바꾼다(그 외 config·μ·Σ·prev_weights·bm 전부 동일).
  **one-way L1 = 0.5·Σ|w_gateON − w_gateOFF| 의 중앙값 ≥ 0.005**
  (§S13.41 P1·§S13.48-B P1(ii)와 **동일 바** — 자의성 0).
  미달 = 게이트 자체가 구조적 no-op → **임계값을 무엇으로 바꾸든 무의미** →
  즉시 SHELVE·축 종결. solver fallback 샘플은 제외·카운트.
- **G2 (게이트 폭 변동성)**: g_t = (μ ≤ 0 인 유효 종목 수)/(유효 종목 수)의
  **sd ≥ 0.04** AND (max − min) ≥ 0.15. 폭이 사실상 고정이면 절대↔분위수 전환
  자체가 no-op이다.

**셋 전부 PASS 시**: 분위수 arm을 **별도 사전등록**한다(이 절에서 arm을 실행하지
않는다). 그때의 파라미터는 `score_gate_mode: 'quantile'`,
**q := 본 사전점검이 산출한 median(g_t)를 소수 셋째 자리 반올림**해 측정 전
정본 고정(§S11.5 λ* 관용구와 동형 — 결과가 아니라 **현행 평균 타이트함**에서
유도). `mega_cap_funding_score_max`는 0.0 **고정**(두 임계 동시 이동 금지).
q 스윕 금지.

**하나라도 FAIL 시**: SHELVE. G1 FAIL이면 **score-gate 축 종결**로 기록한다
(§S13.48-C가 지목한 μ-부호-가로지르기 채널의 유일한 실행 경로가 닫힘).

### 측정 전 정정 (2026-08-21, 실행 전 — 커밋 7aca678 이후)

위 "공통 표본·프레임"이 §S13.48과 동일하다고 적으면서 공분산 입력을
`data.returns_masked`로 상속했는데, 이는 §S13.48-B가 독립 검증에서 지적받은
**SPEC-CODE MISMATCH를 그대로 반복**하는 것이다. **측정 전에 바로잡는다**:

- G1의 재-MVO 공분산 입력을 **production 경로와 동일**하게 바꾼다 —
  `data.raw_returns`를 `returns.index`/`tickers`로 reindex한 패널
  (`backtest.py:1989-1991` → `:1432` `risk_source`).
- 이는 **바(bar) 변경이 아니라 재현 충실도 수정**이며, G0·G1·G2의 임계값
  (0.05 / 0.005 / 0.04·0.15)과 판정 규칙은 **일절 불변**이다.
- 정정 시점: 사전점검 **본실행 전**. 본 정정을 포함한 커밋이 실측보다 앞서므로
  바 선고정은 git으로 계속 독립 입증된다.
- G0·G2는 μ·bm·w만 쓰므로 이 정정의 영향을 받지 않는다.

### 절차/검증 의무

스크립트 1본 `scripts/precheck_s13_49_score_gate_binding.py`(read-only,
백테스트 없음) + 단위테스트(plain 함수, fixture 없음). 실행 선행 검증은
§S13.48과 동일(빈티지 3중 가드). 산출물
`outputs/s13_49_score_gate_binding/summary.json`. 구현은 독립 에이전트 1본
(본실행 금지) → 메인이 단일 foreground 실행 → 독립 검증.

**사전 기대치 기록**: 08-19 1일 관측(경계 종목 0)과 §S13.48-B의 교훈
(제약 집합이 이미 촘촘해 추가 제약이 바인딩하지 않음)을 합하면 **G0·G1 실패가
최빈 기대치**다. 이 점검의 가치는 채택 가능성이 아니라 **축을 싸게 닫는 것**
(45분, 백테스트 0회, 인벤토리 불변)에 있다.

### §S13.49 사전점검 실측 (2026-08-21) — **SHELVE** (G0 PASS / G1 FAIL / G2 FAIL)

실행: `scripts/precheck_s13_49_score_gate_binding.py` 단일 foreground, 424s.
단위테스트 5건 PASS(메인 직접 재실행), 전체 스위트 **588 PASS**(583+5).
빈티지 3중 가드 통과(pkl 2026-08-21 12:15:13 / 워크북 2026-08-19 13:49:14 /
Index 2026-08-21 11:16:45 / last daily_weights 2026-08-19), 실행 전후 불변.
공분산 입력은 위 "측정 전 정정"대로 production `raw_returns` 경로 사용.
인벤토리: read-only — **467 불변**. 산출물
`outputs/s13_49_score_gate_binding/summary.json`.

| 게이트 | 실측 | 바 | 판정 |
|---|---:|---:|---|
| G0 최종 북 바인딩 비율 중앙값 | **0.0638** (n=97) | ≥ 0.05 | **PASS** |
| G1 게이트 ON/OFF 재-MVO one-way L1 중앙값 | **0.00000** (n=23) | ≥ 0.005 | **FAIL** |
| G2 sd(g_t) / range | **0.0353** / 0.1637 | ≥ 0.04 / ≥ 0.15 | **FAIL**(sd 미달) |

→ **종합 SHELVE**(G0∧G1∧G2 전부 필요). 사전등록대로 **score-gate 축 종결**.

**진단(비액션)**: 게이트 대상 수 중앙값 105, 실제 OW 종목 수 중앙값 **28**,
게이트 대상의 `bm − w⁺` 분위수 min **0.00e+00** / p10 **2.58e-11** /
median 3.49e-04 / p90 2.99e-03. median(g_t) = **0.5691**(PASS했다면 arm의 q가
될 값). G1 23샘플 중 **12개가 정확히 0**, 11개가 양수이며 최대
0.0392(2022-12-06)·0.0294(2026-08-19)·0.0134(2019-11-14). 게이트 OFF 시
OW로 풀려나는 종목 수("released OW")는 0~**29**(2025-04-07), 15(2022-03-16),
13(2022-12-06), 12(2026-03-25).

#### 해석 — 게이트는 **간헐적으로만** 바인딩한다

- G0가 PASS했다는 것은 게이트가 **실제로 바인딩한다**는 뜻이다(게이트 대상의
  중앙값 6.4%가 경계에 붙어 있고, p10은 2.58e-11로 사실상 0 gap).
- 그러나 G1의 **중앙값이 0**이다 — 23 샘플 중 12개에서 게이트를 꺼도 북이
  **비트 단위로 동일**하다. 즉 효과가 **희소·간헐적**이다: 대부분의 날은 완전
  slack이고, 소수의 날(레짐 전환기 2020-03, 2022-03/12, 2025-04, 2026-03/08)에
  10~29종목의 OW를 막으며 크게 바인딩한다.
- **중앙값 기준 바(§S13.41·§S13.48-B와 동일하게 채택한 0.005)는 이런 희소
  효과를 잡도록 설계된 통계량이 아니다.** 그러나 이는 **측정 전에 고정한 바**
  이므로 사후에 평균·분위수로 바꾸지 않는다. 판정은 FAIL이다.
- 재도전 자격: 통계량을 "|L1| ≥ τ 인 리밸 **빈도**"로 바꾼 **새 사전등록**이
  필요하다. **단 본 절의 수치(빈도 11/23, 최대 0.039 등)를 그 바의 근거로
  쓰면 선택 편향**이다 — 독립 표본이나 이론적 근거에서 바를 유도해야 한다.
- G2 sd 0.0353 < 0.04(range는 0.1637로 통과)는 게이트 폭이 예상보다 **덜**
  흔들린다는 뜻으로, 절대↔분위수 전환의 여지 자체가 작음을 보강한다.

#### **운영 교훈(중요) — `expected_rebalance.json` 1일 관측은 두 번 연속 오도했다**

§S13.49 사전등록의 "사전 관측 기록"은 08-19 1일에서 **게이트 대상 132종 중
경계 종목 0개**를 관측하고 "구조적 no-op 가능성이 크다 / G0·G1 실패가 최빈
기대치"라고 적었다. **실측은 G0 PASS(중앙값 6.4% 바인딩)로 이를 반증했다.**
같은 날짜의 실제 OW 종목 수도 12였으나 전 구간 중앙값은 **28**이다.

이는 §S13.48-A에서 08-19의 클로백 점유 35.4%가 전 구간 평균 15.2%의 **상위
꼬리**였던 것과 **동일한 실패 양식**이다. 2회 연속으로 08-19 단일 리밸일이
전 구간 분포의 꼬리였다.

**채택 규칙(신설)**: `outputs/operating/expected_rebalance.json`의 단일 리밸일
관측은 **후보의 존재를 발견하는 용도로만** 쓰고, **규모·빈도·바인딩 여부의
...
### §S13.50 실측 (2026-08-22) — **SHELVE** (E0 PASS / P1·P2·P3 FAIL) · 투영 계량 축 종결

실행: schtasks `ai_port_s13_50` 단일 인스턴스, **567s**, exit 0.
빈티지 3중 가드 통과(pkl 2026-08-21 12:15:13 / 워크북 2026-08-19 13:49:14 /
Index 2026-08-21 11:16:45 / last daily_weights 2026-08-19) — 사전등록 기재와 일치.
솔버는 **전 285 solve가 ECOS**(optimizer 95 + 투영 κ=0 95 + 투영 κ=3.0 95),
fallback 0건. 산출물 `outputs/s13_50_alpha_weighted_projection/summary.json`.

**〔추가 공개 2026-08-22, 독립검증 지적 2〕 해 품질**: optimizer는 95/95
`optimal`이나, **투영은 `optimal_inaccurate` 11건 / 10일**(κ=0 7건, κ=3.0 4건)이다.
초판은 "ECOS·fallback 0"만 적고 이 경고를 누락했다 — 런 로그에 cvxpy
`UserWarning: Solution may be inaccurate`가 찍혔음에도 기록하지 않은 것은 결함이다.
해당 10일: 2020-01-13 · **2021-11-18** · **2021-12-17** · 2022-01-17 · 2022-05-13 ·
2022-06-13 · 2024-04-18 · 2024-12-10 · 2026-04-23 · 2026-05-22.
**E0 유일 탈락일(2021-11-18)과 dTO 5위일(2021-12-17)이 여기 포함**되므로,
그 두 날의 이상치는 계량 효과가 아니라 **수치 정밀도 문제일 가능성**이 있다.
결론 불변 근거: 이 10일은 gain 상위 5일과 **교집합 0**이고 기여가
+0.000146 / +0.002199 = **6.6%**에 불과해, 전부 제거해도 P1(3분할 2/3)·
P2(+0.028%p/yr)·P3는 FAIL이다.
인벤토리: read-only 사전점검 — **467 불변**.

| 게이트 | 실측 | 바 | 판정 |
|---|---:|---:|---|
| **E0** median L1(w_0, w⁺) | **1.10e-09** | ≤ 1e-6 | **PASS** |
| **E0** 꼬리 비율 (L1 ≤ 1e-5) | **0.9895** (94/95) | ≥ 0.90 | **PASS** |
| **P1** Σ gain | **+0.002199** | > 0 | (부호 OK) |
| **P1** 3분할 부호 일관 | **2/3** (+0.000804 / **−0.000108** / +0.001503) | 3/3 | **FAIL** |
| **P2** 연환산 gain | **+0.000285 (+0.028%p/yr)** | ≥ +0.0010 | **FAIL** |
| **P3** 리밸당 추가 회전 | **+0.000849** | ≤ 0.0008 | **FAIL** |

→ **종합 SHELVE**. 사전등록대로 **투영 계량 축 종결**(§S13.48-A의 예산 성분 +
본 절의 총 변위로 두 층 모두 소진).

#### E0 PASS — 재사용 가능한 자산이 생겼다

사전등록에 "**E0가 최다 사망 지점**"으로 적었으나 **통과했다**. drift → MVO
(S13.41 대각 스케일 포함) → confidence/eta/no-trade band → L2 투영의
production 리밸 경로를 오프라인에서 **중앙값 L1 1.10e-09**로 재현한다.
1e-5를 넘긴 날은 **2021-11-18 단 1건(1.003e-05, 바의 0.3% 초과)**뿐이다.

이것은 본 절의 판정과 별개로 **방법론적 소득**이다: 이제 가중치 구성 계층의
반사실은 **프록시 없이 직접** 잴 수 있다. §S13.48-A가 쓴 "군집을 μ 상·하위로
갈랐을 때의 전진수익 차" 같은 대리 지표는 더 이상 필요 없다.

#### 왜 상금이 사전 상한보다도 작았나 — **변위는 알파-불가시이되 자유롭지 않다**

§S13.48-A는 예산 성분만으로 **+0.054%p/yr**를 추정했다. 본 절은 대상을
**총 변위**(예산 + TE 캡 + 섹터 + norm1 + per-stock + score gate + mega)로
넓혔는데도 **+0.028%p/yr** — 오히려 **절반**이다. 프록시가 낙관적이었다.

원인은 진단에 직접 찍혔다:

- 투영 변위 `‖w_0 − candidate‖₁` 중앙값 **0.01837**(북의 1.84%)
- 계량을 κ=3.0으로 바꿔 실제로 움직인 양 `‖w_κ − w_0‖₁` 중앙값 **0.00348**
- **계량 자유도 = |w_κ−w_0| / 변위 : 중앙값 0.1965 · 평균 0.1981 · 최대 0.3485**

즉 **변위의 약 80%는 제약 집합이 강제한 것이고, 계량이 손댈 수 있는 몫은
~20%뿐**이다. §S13.48-A가 세운 "리밸 회전의 35%가 알파-불가시 균일 이동이므로
그 방향을 알파 쪽으로 돌리면 수확이 있다"는 프레임은 **전반부(알파-불가시)는
맞고 후반부(따라서 재배분 가능)는 틀렸다**. 알파-불가시인 것과 재배분 가능한
것은 다르다 — 대부분의 변위는 제약이 정한 유일해다.

#### 그 +0.028%p/yr 조차 5일이 전부다

- **gain 상위 5일 합 +0.002214 = 전체 +0.002199의 101%** — 나머지 89일은
  합쳐서 **−0.000015**(정확히 상쇄).
- 양수일 **53/94 = 56.4%** — 동전던지기 대비 유의하지 않다.
- 상위 5일: 2026-01-26(+0.000850) · 2025-01-08(+0.000433) · 2020-04-09
  (+0.000350) · 2020-02-11(+0.000294) · 2025-04-07(+0.000287).

#### P3 — 비용도 무료가 아니다(가까스로 초과)

`mean_t(dTO)` **+0.000849** vs 바 0.0008 — **6% 초과 FAIL**. 분포는 극심한
우편향(중앙값 5.46e-05, p90 +0.00405, 최대 +0.00830 @2020-09-03)이고
**상위 5일이 총 추가회전의 43%**를 차지한다. `dTO > 0.001`인 날은 16/95.
one_way_tc 0.0010 기준 비용은 0.000849 × 12 × 0.0010 = 1.0184e-05
= **0.10bp/yr**이므로 순이득은 0.0285 − 0.0010 ≈ **+0.027%p/yr**.
IR 1.83 북에서 무의미하다.
**〔정정 2026-08-22, 독립검증 지적 1〕** 초판은 이 값을 "≈1.0bp/yr → 순이득
+0.018%p/yr"로 적었다. **비용을 10배 과대 계상한 산술 오류**다(arm에 불리한
방향이었으므로 P3 FAIL·SHELVE 판정은 불변). 비용은 거의 0에 가깝고,
**이 축이 죽은 이유는 비용이 아니라 상금 자체가 없다는 것**이다.
덧붙여 최대 변위일이 이득일과 겹치지 않는다(2021-12-17은 dTO 상위 5일이면서
gain 하위 4일) — 계량 변경이 신호가 아니라 잡음을 증폭한 날이 섞여 있다.

#### 기록 정정 — `n_skipped`는 1건 과다 계상

`summary.json`의 `n_skipped: 3`은 **틀렸다**. `skip_reasons`의 세 항목 중
`no_fwd_window(gain only)`(script:462)는 **행을 버리지 않고** gain=NaN으로
남긴 뒤 `rows.append`로 진행하는 경로라 "skip"이 아니다. 실제로 탈락한
리밸일은 **2건**(`no_prev_daily` 1 · `optimizer_fallback` 1)이며,
**95(사용) + 2(탈락) = 97(전체)**로 맞는다. `rows.jsonl` 95줄, `n_gain` 94로
교차검증된다. 게이트 판정에는 영향 없다(E0는 95, P1/P2는 94 표본을 쓴 것이
사전등록 정의와 일치). 카운터 라벨만 오도적이다.
`optimizer_fallback` 1건은 production이 fallback한 날을 재현하지 않고
보수적으로 제외한 것으로, E0를 인위적으로 유리하게 만들지 않는다.

#### 절차 공개 (정직성 기록)

1. **중단 2회**: 08-21 foreground 런(~22/97에서 사망)과 08-22 13:04 schtasks
   런(`LastResult 3221225786` = `STATUS_CONTROL_C_EXIT`, 출력 0바이트)이
   연속 실패했다. 등록 XML 점검 결과 `<StopOnIdleEnd>true</StopOnIdleEnd>`
   (Task Scheduler 기본값)를 덮어쓰지 않은 것이 유력 원인이라
   `StopOnIdleEnd=$false`·`RestartOnIdle=$false`·`RunOnlyIfIdle=$false`로
   재등록했다. **이 진단은 확증이 아니라 최유력 가설**이다(운영 이벤트 로그
   비활성으로 종료 사유를 직접 확인하지 못함).
2. **체크포인트 도입(커밋 3294a01)**: 진단에 의존하지 않도록 리밸일 레코드를
   `rows.jsonl`에 즉시 flush+fsync하고 재시작 시 재사용하도록 했다. 레코드는
   리밸일마다 독립·결정적이므로 **측정값 불변**이며 b606d1f의 게이트도 불변이다.
   사전등록(b606d1f)·스크립트(b301c9a) 이후의 변경이므로 여기 명시한다.
   결과적으로 재개 없이 1회 완주했다(체크포인트 미사용).
3. **부분 출력 관측**: 08-21 사망한 런의 ~22/97 리밸 중간 출력을 메인이
   화면에서 봤다(주로 `L1repro` 1e-10~1e-7). 게이트는 그보다 앞선 b606d1f에
   커밋되어 있어 바를 바꿀 수 없었고, 실제로 바꾸지 않았다. 다만 그 관측이
   "E0는 통과할 것"이라는 기대를 형성한 것은 사실이므로 기록한다.
   E0 이외 게이트(P1/P2/P3)의 값은 그 시점에 집계된 바 없다.

#### 결론

- **투영 계량 축 종결.** 예산 성분(§S13.48-A)·총 변위(§S13.50) 두 층 모두
  소진. 재도전하려면 "계량을 바꾼다"가 아니라 **제약 집합 자체를 바꾸는**
  새 사전등록이 필요하다(그것은 리스크 캐릭터 변경이므로 §2.5 게이트 대상).
- **이번 라운드(비선형 2차) 누적**: 사전점검 **5축 전부 SHELVE**
  (§S13.48 A/B/C · §S13.49 · §S13.50), arm **0건**, 인벤토리 **467 불변**,
  production **무변경**.
- **§S13.12 상한 재확인**: 가중치 구성 계층에서 "무료 재배분"을 찾으려는
  시도는 이로써 3회 연속(§S13.48-A · §S13.49 · §S13.50) 실패했다.
  구조적 우회 3경로(①리스크 계층 단독 ②무비용 노출 재배분 ③objective-북 정렬)
  중 **②의 구체적 실행 경로는 모두 소진**된 것으로 본다.

#### §S13.50 검증 기록 (2026-08-22)

**검증 주체 고지 〔정정 2026-08-22〕**: 독립 검증자(`verify1350`)의 유휴 전환을
보고 실패로 오판해 메인이 V1~V6을 병행 재실행했으나, **검증자는 이후 전 항목
보고서를 반환했다**. 따라서 이 절의 검증은 **독립검증 + 메인 재현의 이중 검증**이며,
"자기검증이라 독립성이 약하다"는 초판 서술은 **철회**한다.
독립검증은 메인이 놓친 **결함 2건을 적발**했다(아래 §S13.50 본문의 정정 2건).
메인의 병행 재계산과 검증자의 독립 재계산은 게이트 4종 전 수치가 일치했다.

| 항목 | 내용 | 판정 |
|---|---|---|
| V1 | b606d1f의 바(E0 ≤1e-6·≥90%@1e-5, P1 sum>0∧3/3, P2 ≥0.0010, P3 ≤0.0008)가 summary.json threshold와 일치. b606d1f..HEAD에서 결정 로그 **삭제 줄 0** — 사전등록 문단 무수정 | **PASS** |
| V2 | `rows.jsonl` 95줄만으로 게이트 4종 독립 재계산 → E0 median 1.096114e-09·tail 0.989474, P1 sum +0.002198692·3분할 [+0.000804, −0.000108, +0.001503], P2 +0.000284576, P3 +0.000848659 — **summary.json과 전부 일치** | **PASS** |
| V3 | 서술 주장 (a)~(g) **7건 전부 CONFIRMED** (자유도 0.1965/0.1981/0.3485, top5 101%·나머지 −0.000015, 53/94, dTO 43%·16/95, E0 fail 2021-11-18 1.0033e-05 단건, disp 0.01837·0.00348, 95+2=97) | **PASS** |
| V4 | 이번 런은 `[resume]` **0회 — 캐시 미사용 1회 완주**(rows.jsonl 최종 쓰기 13:30:37 = EXIT 시각). 따라서 3294a01은 이번 측정에 개입 자체를 안 했다 | **PASS**(단서 아래) |
| V5 | `float(kappa)==0.0` 명시 분기가 원식 `sum_squares(w − candidate)`를 탐(script:178-183) · `KAPPA=3.0` 단일 상수, 스윕 루프 없음 · 공분산 입력 `data.raw_returns`(script:323) · ABORT 가드 6개 | **PASS** |
| V6 | 전체 스위트 **595 passed** · `git status` clean · `git diff ded6585..HEAD -- src/ variants/` **비어 있음**(production 무변경) | **PASS** |

**V4 잠재 결함(이번 측정 무영향, 향후 주의)**: 재개 경로
`if key in done: rows.append(done[key]); continue`는 `_skip()`을 우회한다. 캐시된
행은 정의상 탈락이 아니므로 대부분 무해하나, **`no_fwd_window(gain only)` 라벨만은
캐시 시 유실**된다(그 행은 append되면서 동시에 `_skip`도 호출하는 이중 성격이라).
즉 **재개된 런의 `n_skipped`는 과소 계상**될 수 있다. 게이트 4종은 모두 `rows`와
유한 gain에서 계산되므로 **판정에는 영향 없다**. `no_prev_daily`·
`optimizer_fallback` 날짜는 애초에 체크포인트에 안 남으므로 재개 시 정상 재계상된다.
근본 원인은 §S13.50 본문의 "n_skipped 1건 과다 계상"과 **같은 결함**(하나의 카운터가
'탈락'과 '부분 사용'을 겸함)이다. 재사용 시 카운터를 분리할 것.

**V4 두 번째 잠재 결함(독립검증 지적, 미발현)**: `rows.jsonl`에 **빈티지·코드
스탬프가 없다**. 따라서 다른 빈티지·다른 스크립트 버전에서 만들어진 구
`rows.jsonl`이 남아 있으면 **무검증으로 재사용**된다. 이번 런은 캐시 미사용이라
발현하지 않았으나, 이 체크포인트 패턴을 재사용할 때는 **파일 헤더에
(pkl mtime, 워크북 mtime, Index mtime, 스크립트 git sha)를 기록하고 불일치 시
폐기**하도록 반드시 보강할 것. 빈티지 3중 가드(§S13.47)의 취지가 체크포인트
경로에서 우회되면 안 된다.

---

## 비선형 2차 라운드 종료 (2026-08-22, 사용자 지시)

사용자 지시로 §S13.48~50 라운드를 **종료**한다.

**성적**: 사전점검 **5축 전부 SHELVE**, arm **0건 실행**, 인벤토리 **467 불변**,
production **무변경**(마지막 src/variants 변경은 §S13.47 승격 `ded6585`).

| 축 | 대상 | 결과 |
|---|---|---|
| §S13.48-A | 투영 계량 알파 맹목성(예산 성분) | SHELVE — 진단 참, 규모 미달(+0.054%p/yr) |
| §S13.48-B | RMT 벌크 고유값 정제 | SHELVE — 전제 반증(floor 방향 액티브분산 3.89e-06) |
| §S13.48-C | mega funding 히스테리시스 | SHELVE — §9 발동, 사전등록 메커니즘 부재 |
| §S13.49 | score-gate 분위수 전환 | SHELVE — 간헐 바인딩(G1 중앙값 0) |
| §S13.50 | 투영 계량 재설계(총 변위) | SHELVE — 계량 자유도 0.1965, 상금 +0.028%p/yr |

**라운드가 남긴 것(arm 0건이어도 순소득이 있다)**:
1. **E0 재구성 자산** — production 리밸 경로를 오프라인에서 median L1 1.10e-09로
   재현(§S13.50). 앞으로 가중치 구성 계층 반사실은 **프록시 없이 직접 측정** 가능.
2. **구조 지식 3건** — ① 투영 변위는 알파-불가시이되 **자유롭지 않다**(80%가
   제약 강제 유일해) ② Σ 고유공간의 랭크 결핍 방향에 리스크 무료 적재는 이
   제약집합에서 **기우** ③ score gate는 **레짐 전환기에만** 바인딩.
3. **운영 규칙 2건** — ① `expected_rebalance.json` 1일 관측은 **후보 발견용만**
   (2회 연속 꼬리에 속았다) ② **사전등록은 측정 전 단독 커밋**.
4. **§S13.12 상한 재확인** — 구조적 우회 3경로 중 **②무비용 노출 재배분의
   구체적 실행 경로는 소진**(3연속 실패: §S13.48-A·§S13.49·§S13.50).

**다음 라운드를 열 때의 출발점**: 가중치 구성 계층이 아니라 **제약집합 자체**
(TE 캡·active share L1 구조)를 바꾸는 방향. 이는 리스크 캐릭터 변경이므로
§2.5 게이트 대상이며 성격이 다른 결정이다. 저가치 잔존 후보(21BD 예측평균 —
⚠ `res.raw_predictions`는 lag 1BD 기적용이라 재주입 시 이중지연,
label_gain 평탄화 — booster 해시 P0 선행)는 상금 기대치 근거가 없다.

### 라운드 종료 시점 북 상태 점검 (2026-08-19 리밸 기준, 비액션 관측)

라운드 종료 보고 과정에서 `outputs/operating/*`를 읽어 아래를 관측했다.
**액션 아님 — 다음 세션의 착수 후보로만 기록**한다.

- **Technology 액티브 리스크 점유 90.6% > 가드 85%** —
  `risk.json.guardrails.top_sector_active_risk_breached = **true**`.
  §S10에서 0.75→0.85로 재캘리브레이션해 해소했던 항목이 **다시 위반**이다.
  OW 9종 중 5종(MU·000660·005930·SNDK·285A)이 메모리/스토리지 반도체이고
  이들 합이 **+7.58%p = 전체 OW 예산 12.72%p의 60%**다.
  단 섹터 **비중** 편차 한도(0.10)는 위반 아님 — 위반은 **리스크 점유** 축이다.
- 종목 액티브 리스크 점유 최대 000660 **30.6%** < 가드 35%(미위반).
- ex-ante TE 3.81% < 가드 4.5%(미위반, 헤드룸 0.69%p). 커밋된 최적화 캡은 3.5%.
- optimizer_failure_rate **5.15%**(194 solve 전부 ECOS, fallback 0).
- 데이터 신선도 정상(tail_ffill 1일 ≤ 10).
- **데이터 품질 확인 요망**: `DTE`의 섹터가 `Communication Services`로 매핑돼
...
## S15 fix-pack 측정 결과 — 정확성 수정 8건 ON, ΔIR +0.166 (2026-08-27)

**실행**: schtasks 일회성 13:00 → 13:19 exit 0(순수 연산 ~19분).
`run_variant.py --variant variants/s15_fixpack.yaml --no-cache`. 빈티지 쌍
런 전후 불변(ai_signal_data 08-25 20:47:55 / Index 08-25 16:37:36) —
**S0(250) 1.3811과 동일 빈티지 직접 비교 유효**. ECOS **194/194 solve·
fallback 0**. 구현 커밋 38d0f6d(fix-pack)·5a4142b(가드)·9d26857(ops),
전체 스위트 **646 PASS**(fix-pack OFF 파리티 테스트 포함).

| 항목 | S0(250) | fix-pack ON | Δ |
|---|---:|---:|---:|
| IR | 1.3811 | **1.5466** | **+0.1655** |
| TE | 3.63% | 3.73% | +0.10%p (≤4.5% 가드) |
| Active return | 5.01% | 5.78% | +0.77%p |
| Turnover(two-way) | 71.0% | 69.8% | −1.2%p |
| realized_beta | 1.049 | 1.051 | +0.002 |
| avg_ic | 0.0196 | 0.0194 | −0.0002 (사실상 불변) |
| MaxDD | −32.1% | −32.5% | −0.4%p |
| 퇴화율 | 12/33 | 14/33 | +2 (report-only 트랙) |
| 서브 IR P1/P2/P3 | 1.440/0.944/1.753 | 1.624/0.939/1.923 | +0.18/−0.01/+0.17 |
| P4_tail(신설) | — | 1.587 | 커버리지 2026-08-25까지 |

**해석**: avg_ic 불변 상태에서 IR +0.166·turnover −1.2%p — 알파 랭킹이 아닌
**구조 결함 제거 경로**의 개선(가짜 신호 제거가 회전을 줄이고, 유효 예측
보존·리스크 모델 대각 정합이 북 구성 품질을 올림). 서브는 P1/P3 뚜렷한 양,
P2 −0.006(사실상 0). 퇴화 +2는 피처 패널 변화(팩터 캘린더 채움·mom_accel
완전창·롤오버 연장)로 early stopping 경로가 달라진 부수 효과 — report-only.

**판정(사전등록 준수)**: 채택 기준은 **정확성**이며 8건 전부 코드 추적 +
적대적 검증으로 확인된 실결함이므로 **fix-pack ON을 production 채택 권고**.
ΔIR +0.166은 관측 기록(선택 이벤트 아님 — **인벤토리 467 불변**, §S13.46 PM
recert 선례). **활성화는 §8대로 사용자 결정**: production variant에
...
## S15.1 결과 — monotone 마진 arm E1 FAIL, 불채택 (2026-08-27)

**실행**: schtasks 13:40 → 14:06 exit 0(~26분). ECOS **194/194·fallback 0**.
빈티지 쌍 런 전후 불변(08-25 20:47:55/16:37:36) — S0′ 1.5466 직접 비교 유효.

| 항목 | S0′(fix-pack ON) | monotone arm | Δ |
|---|---:|---:|---:|
| IR | 1.5466 | 1.5472 | **+0.0006** |
| TE | 3.73% | 3.73% | −0.01%p |
| Turnover(two-way) | 69.8% | 69.0% | −0.8%p |
| avg_ic | 0.0194 | 0.0179 | −0.0015 (−7.9%) |
| MaxDD | −32.5% | −33.1% | −0.6%p |
| 퇴화율 | 14/33 | **12/33** | −2 개선 |
| 서브 P1/P2/P3 | 1.624/0.939/1.923 | 1.549/1.019/1.939 | −0.075/+0.080/+0.017 |
| P4_tail | 1.587 | 1.553 | −0.034 |

**판정**: ΔIR +0.0006 ≪ +0.36 바, 서브 부호 비일관(P1 음) → **E1 FAIL,
불채택. 플래그 default-OFF 유지, production 무변경.**

**관측(비액션)**: ① 제약이 퇴화율을 14→12/33로 낮춤 — §S13.8의 "암묵적
용량 규제자" 계열 효과(단조 제약이 과적합 분할을 차단). ② avg_ic −7.9%
하락에도 IR 보합 + turnover 감소 — 마진 축의 단변량 예측력(t 6.9)은 이미
모델이 소화한 정보이며, 제약은 노이즈 분할만 제거(무해·무익). §S13.12
전달 상한과 정합. ③ 비선형 정칙화 축(monotone)은 이 맵 기준으로 소진 —
다른 맵 재시도는 근거 없는 스윕이므로 금지.

**회계**: 성능 arm 1건 실행 → **인벤토리 467→468**. DSR 해킷은 채택 후보
...
## S15.2 측정 결과 — E1·E2 전부 PASS, 정확성 근거 채택 권고 (2026-08-28)

**실행**: schtasks 10:06 → 10:32 exit 0(~26분, elapsed 1574.9s). ECOS
**194/194·fallback 0**. 빈티지 쌍 런 전후 불변(08-25 20:47:55/16:37:36) —
S0′ 1.5466 페어드 비교 유효. 런 로그에 가드 발동 확인:
"S13.41 fix: iv30_z observed coverage 88.9% (imputed cells forced inert)".

| 항목 | S0′(fix-pack ON) | optvol scale fix | Δ |
|---|---:|---:|---:|
| IR | 1.5466 | 1.5356 | **−0.0109** |
| TE | 3.734% | 3.727% | −0.007%p |
| active_share | 19.92% | 19.94% | +0.01%p |
| Turnover(two-way) | 69.82% | 69.75% | −0.07%p |
| **avg_ic** | 0.0193779842 | 0.0193779842 | **비트 동일** |
| realized_beta | 1.0507 | 1.0515 | +0.0008 |
| MaxDD | −32.46% | −32.46% | +0.01%p |
| 퇴화율 | 14/33 | **14/33** | 동일 |
| 서브 P1/P2/P3 | 1.624/0.939/1.923 | 1.610/0.946/1.891 | −0.015/+0.007/−0.032 |
| P4_tail | 1.587 | 1.647 | **+0.060** |

**게이트 판정**:
- **E1 구조 불변식 PASS** — avg_ic 비트 동일 + 퇴화 14/33 동일 + ECOS
  194/194: 수정이 알파·모델 경로에 일절 비접촉, Σ 대각만 변경했음이 증명.
- **E2 do-no-harm PASS** — 서브 부호 혼합(P2·P4_tail 개선, P1·P3 소폭 하락),
  일관 음 아님. 캐릭터 전 항목 보존(TE·active share·turnover·beta·MaxDD
  전부 노이즈 수준). §S13.41 채널 재심 트리거 미발동.

**해석**: ΔIR −0.011은 |Δ| ≪ 0.36 노이즈 대역. 즉 **§S13.41의 +0.124
채택 근거 중 버그(가짜 median-z 스케일 3.77% 셀 + dense 리스크 원천)가
기여한 몫은 사실상 0** — 채널 이득은 아티팩트가 아니었음이 역으로 입증됨.
수정은 "결과를 바꾸지 않으면서 계약을 복원"하는 순수 정확성 수정으로 판명.

**권고**: §S13.33·§S15 fix-pack 전례에 따라 **정확성 근거 채택 권고**
(IR 델타는 관측일 뿐 선택 게이트 아님). flip 시 새 S0′ = **1.5356**
(1.5466 은퇴·혼용 금지). 활성화는 §8 사용자 결정 대기.
...
## S16 구조 리뷰 트랙 결과 + O-트랙 수정 (산출물 불변) — 2026-08-31

**리뷰 실행**: 멀티에이전트 리뷰(모듈 10군 + 횡단 5렌즈 = 15 리뷰어, src 전량 + ops
스크립트 + 진단 스크립트, 체크리스트 = quant-code-review) → 원시 61건 → 중복 제거 51건
→ 발견별 반박 우선 검증자 2~3인(코드 추적 · production 영향 정량 · 경험적 재현) →
**확정 47건 / 반박 1건 / §S15 기지 중복 3건**. 총 138 에이전트·897 도구 호출.
전체 보고서 = `outputs/reports/2026-08-31-structural-review-s16.md`
(아티팩트 판본 https://claude.ai/code/artifact/63c20f22-4b30-4448-b2b7-e0f3ec1ab421).

**메인 직접 재현 항목**(에이전트 주장을 그대로 채택하지 않고 pkl·pytest·워크북으로
독립 관측):

| ID | 실측 | 값 |
|---|---|---|
| O1 | 라이브 모델 나이 (models 객체 동일성 추적) | **354일** (2025-08-28 적합본, 4회 연속 재사용, 고유 19/33) |
| O2 | `pytest tests/acceptance -q` | 5 failed → 이후 `tests/test_residual_sleeve.py`에서 **6번째** 동일 결함 발견 |
| P1 | `tg_upside` 티커별 중앙값 최저 8개 | LN 8종 −3.86~−3.89, 9위 TSLA −0.69 (3.2σ 단절) |
| P1 | 워크북 직접 조회(08-25 빈티지) | AZN PX_LAST **12,170**(GBp) vs TG **159.31**(GBP) — 배수 정확히 100 |
| P2 | `best_calculated_fcf_level_z` | 000660·005930 +5.00 클립, **KR·JP 제외 234종 std 0.001** |
| P3 | `cash_conversion_z` 상위 | 9432 +5.00 · NVDA +4.78 · AAPL +3.29 — 발행주식수 순 |
| P4 | mc_* 상관 (3개 날짜) | \|corr(mc_rate_x_eps_rev, eps_rev)\| = **1.0000** (부호만 날짜별로 반전) |
| O5 | 캘린더 밀도 | panel 3,283행/12.58년 = **260.9행/년** (연율화 상수는 252) |
| T1-3 | MVO 목적함수 항 비율 | 리턴항 ≈0.88 vs 리스크항 상한 4.86e-06 → **약 18만배** |
| P4 | 매크로 스칼라 절대값 중앙값 | rate 1.106 · dxy 1.144 · vix 0.850 · **slope_raw 0.589**(미z화) / slope_z 1.126 |

**O-트랙 수정 (커밋 5d3df05·1891189, 산출물 불변)**

- **O2** — §S15.2 flip이 핀 테스트 allowlist를 갱신하지 않아 HEAD가 red였다
  (acceptance 5건 + `test_residual_sleeve` 1건). §S15 flip 선례대로 allowlist 흡수,
  역사적 arm variant는 사전등록 상태 보존을 위해 무변경. **재발 방지로 CLAUDE.md §8에
  flip 체크리스트 5항목 고정**(핀 테스트 갱신을 같은 커밋에서 처리). 674 passed.
- **O3** — `export_operating_data._load_optvol_scale`이 §S15.2 이전 레시피(임퓨트 dense
  패널·마스크 없음)로 남아 `risk.json`·monitoring TE 감사·`expected_rebalance`의 Σ가
  production과 달랐다. `src/backtest.py` L2088-2116과 동일 분기로 정합. OFF 바이트 동일.
- **O4** — 번들의 run_dir resolved config vs 현재 variant overrides 드리프트 검사 신설
  (report-only). **실측 즉시 포착**: production 번들이 `s15_fixpack_enabled`·
  `option_vol_scale_fix_enabled` 2건 드리프트 — 두 flip 이후 production 디렉터리가
  재실행되지 않아 대시보드가 은퇴 기준선 IR 1.3811을 읽고 있었다. 정당한 일시 드리프트를
  깨지 않도록 raise 금지. 레지스트리 출력은 타임스탬프 외 불변.
- **O7** — `metrics.json`·`experiment_manifest.json`에 (워크북, Index.xlsx) mtime/size
  지문 기록(`data_vintage`). §S13.47 이후의 동일 빈티지 규율을 산출물만으로 검증 가능하게
  함. sha256은 비용 대비 무의미해 미채택.
- **O8** — `ESSENTIAL_SHEETS` 13개 중 하나가 통째로 없어도 유니버스 가드가 250/250으로
  통과하던 사각 해소. `expected_universe_size`가 설정된 런에서만 raise(합성 픽스처 보호).
- **E0** — `expected_rebalance`가 리밸일 as-of에 실제 `portfolio_weights`를 재현하는지
  L1로 자기검증해 `e0_selfcheck`로 발행(report-only, §S13.50 재현 자산 활용).

**미조치로 기록만 하는 항목**: O5(연율화 252 vs 260.9 — 모든 arm에 일관 적용되므로 ΔIR
비교는 유효, 절대 임계 판정에만 편향. 수정 시 전 역사 수치 이동이라 §8 대상),
O6(DSR 생존편향 항목 구조적 영구 FAIL + 기대 최대 SR 근사 과대), O9(진단 스크립트군의
execution-lag 이중 적용), D군 12건(휴면 경로 — 해당 플래그를 켜기 전 선결).

---

## S16.1 사전등록 (Tier 0 단위·중복 정확성 팩 — 측정 전 단독 커밋) — 2026-08-31

**대상**: 코어 65피처 중 **7개가 전 종목에서 의도한 정보를 담지 못하고, 1개가 8종목에서
상수 더미**인 상태를 해소한다. 단일 default-OFF 플래그 `s16_unit_fixpack_enabled`
(§S15 fix-pack이 8건을 한 플래그로 묶은 선례).

| # | 수정 | 위치 | 내용 |
|---|---|---|---|
| P1 | LN 가격 단위 | `data_loader.PRICE_UNIT_SCALE = {"LN": 0.01}` | LSE는 GBp(펜스) 호가, FactSet 목표주가는 GBP. `_apply_usd_conversion` 이전에 적용해 `local_prices`·`prices` 양쪽 정합. TG/PX 비율 [0.2,5] 밖 경고 가드 동반(플래그 무관 항상) |
| P2 | FCF·CAPEX 레벨 z | `accounting.py:50` | 현지통화 절대금액 CS z → `cross_sectional_zscore(rolling_tsz(raw, 756, 252))`. 같은 파일 72-73행 VALUATION_SHEETS 관용구와 동일(REDESIGN M 주석이 서술한 실패 모드). `BEST_ROE`(%)는 무변경 |
| P3 | 현금전환 단위 | `accounting.py:116` | `fcf/\|eps\|`(총액÷주당 = 주식수 프록시) → `fcf/\|eps × shares\|`, `shares = market_cap / prices`(둘 다 USD → 정확한 주식수). **P1에 의존**(LN prices가 100배면 주식수가 100배 작음) — 같은 플래그라 정합 |
| P4 | 매크로 이중 z | `assembly.py:777` + `macro_cross.py:138` | MacroCross 그룹을 `skip_zscore`에 추가. 동시에 `slope_panel`을 다른 세 스칼라와 같은 `_rolling_zscore(·, 63)`로 z화(외부 z가 가려온 스케일 불일치 — **window 63 동일이라 새 자유도 0**) |

**비교 기준**: S0′ **IR 1.5356** / TE 3.73% / β 1.052 / 퇴화 14/33 / avg_ic 0.019378,
빈티지 (2026-08-25 20:47:55, 2026-08-25 16:37:36). 런 전후 빈티지 확인 의무.

**측정 계획**: `variants/s16_1_unit_fixpack.yaml` = production 전체 + 플래그 1줄.
ECOS 단일 프로토콜, `--no-cache`, schtasks 일회성(`outputs/run_variant_task.ps1` —
ES_SYSTEM_REQUIRED 슬립 억제 + 빈티지 전후 기록).

**채택 기준 = 정확성** (§S13.33 데이터 정확성 4건·§S15 fix-pack 선례). 4건 모두 실측으로
확인된 결함의 수정이므로 **ΔIR 부호와 무관하게 채택 후보**이며, ΔIR·ΔTE·Δbeta·Δ퇴화율·
Δturnover는 관측 기록이다. **선택 이벤트가 아니므로 DSR 인벤토리 비계수**(468 불변,
§S13.46 PM recert·§S15 fix-pack 선례).

**§S15 fix-pack과의 결정적 차이 — 판정 기준을 바꾼다**: §S15는 구조 경로만 수정해
`avg_ic` 비트 불변을 구조 불변식(E1)으로 쓸 수 있었다. **S16.1은 알파 경로(피처 정의)를
직접 수정하므로 `avg_ic`가 반드시 바뀐다.** 따라서 E1은 avg_ic 불변이 아니라 아래로 한다.

- **E0 (파리티, 측정 전 선행)**: 플래그 OFF에서 4개 수정 각각이 바이트 동일임을 단위
  테스트로 인증(`tests/test_s16_unit_fixpack.py`). 전체 스위트 0 failed.
- **E1 (정확성 확증)**: 산출 패널에서 아래가 **전부** 성립해야 한다. 미달 시 구현 결함으로
  간주하고 수정 후 재측정(성능 게이트가 아님).
  1. `tg_upside`의 LN 8종 중앙값이 −3.8σ 상수에서 벗어나 비-LN 분포와 겹칠 것
  2. `best_calculated_fcf_level_z`의 KR·JP 제외 종목 표준편차가 0.001 → **0.5 이상**
  3. `cash_conversion_z` 상위 6종이 발행주식수 순서와 일치하지 않을 것
  4. 3개 표본일에서 `|corr(mc_rate_x_eps_rev, eps_rev)| < 0.999`
...
- **관측(사전 예측 포함)**:
  - P2의 `rolling_tsz(min_periods=252)`는 각 티커의 첫 251 **유효** 관측을 NaN으로
    ...
    다만 **기존 VALUATION_SHEETS level_z 4종이 같은 관용구로 이미 동일한 워밍업을
    갖고 있으므로** 새로운 종류의 처리는 아니다.
  - P4의 클립 저촉률은 **사전 추정 0.6~0.8%**(매크로 스칼라 분포 실측 + 레그 정규
    근사) — `[MacroCross] skip-CS-z ON: pre-clip |z|>5 cells` 로그로 실측되며 이와
    크게 다르면 보고 대상.

**flip은 측정 후 §8 사용자 결정.** 채택 시 S16.1 ON 수치가 새 S0′가 되며, 이후 §S16.2(P5)·
Tier 1 arm은 이 기준선 위에서 단일 플래그로 측정한다.

---

## S16.4 사전점검 — T1-2 공분산 수축 (arm 미실행 · **SHELVE**) — 2026-08-31

**가설(§S16 P6)**: `estimate_covariance`는 lookback 창에 NaN이 하나라도 있으면 무조건
`_pairwise_covariance`(수축 없음)로 가고 NaN이 전혀 없을 때만 Ledoit-Wolf를 쓴다
(`portfolio_optimizer.py:88`). production 리스크 원천은 `raw_returns`(상장 마스킹·임퓨트
없음)이고 2019~2025년에 24종이 순차 상장했으므로 **97회 리밸 전부에서 126×250 창에 NaN이
존재**한다 — LW 분기는 죽은 코드다. 결과적으로 rank ≤ 125이고 나머지 125개 고유값은
`fallback_var × 1e-4`(연 vol 약 0.3%)로 바닥 처리되므로, **바닥 처리된 부분공간 방향의
액티브 베팅이 사실상 공짜로 취급되어 ex-ante TE가 실제 리스크를 과소평가**할 것이다.

**사전점검 (재실행 0 — 기존 산출물만 사용)**: ex-ante TE가 실제로 다음 보유기간의 액티브
리스크를 과소예측하는지 페어드로 검정한다. ex-ante는 문제의 추정기로 만든 값이고 실현은
그 시점 기준 표본 외이므로, 이 비율은 정당한 **표본 외 보정(calibration) 검정**이다.

**표본 정합 주의(자기정정)**: 최초 계산은 ex-ante 계열을 `operating_codex_causal_rank_65/
monitoring.json`(source run = `outputs/codex_causal_rank_65`)에서, 실현 계열을
`outputs/s15_2_optvol_scale_fix` pkl에서 가져와 **서로 다른 런의 북을 짝지었다**. 두 런은
fix-pack·optvol 수정으로 북이 다르므로 무효다. 아래는 **동일 런**(둘 다
`outputs/codex_causal_rank_65`, IR 1.3811, 전체표본 실현 TE 3.63%)으로 다시 검정한 결과다.

**실측 (동일 런 페어, n=96)**

| 지표 | 값 |
|---|---:|
| ex-ante TE 중앙값 | 0.0320 |
| 실현(향후 21BD) 중앙값 | 0.0315 |
| 실현/ex-ante 비율 중앙값 | **0.994** |
| 비율 > 1 인 리밸 비중 | **49.0%** |
| log-비율 평균 | +0.0047 |
| log-비율 t값 | **+0.15** |
| 3분할 비율 중앙값 | 0.899 / 1.025 / 1.029 (방향 비일관) |
| TE 캡(0.035) 바인딩 비율 | 26.8% |

(참고: 잘못된 교차-런 페어링에서는 t=+1.26이었다 — 정정 후 편향이 더 줄었으므로 결론은
같은 방향으로 **강화**되었다.)

**판정: SHELVE (전제 반증).** 수축 없는 pairwise Σ는 21BD 지평에서 **편향 없이 보정되어
있다**(t=+0.15, 부호 분포 동전 던지기, 3분할 방향 비일관). 전체표본 실현 TE 3.63%와
ex-ante 중앙값 3.20%의 차이는 추정기의 편향이 아니라 "21BD 창 변동성의 평균" vs
"전체표본 변동성"의 정의 차이(시변 변동성)에서 온다. **바닥 처리된 부분공간이 착취되고
있다면 ex-ante가 실현을 체계적으로 과소예측해야 하는데 그렇지 않다** — §S13.48-B(RMT 벌크
정제, floor 방향 액티브분산 점유 3.89e-06으로 전제 반증·SHELVE)와 서로 독립인 두 번째
경로로 같은 결론에 도달했다.

**남는 것(비액션 기록)**: P6의 코드 품질 지적 자체는 유효하다 — `estimate_covariance`의
docstring "via Ledoit-Wolf shrinkage"가 production 경로를 오도하고, LW 분기는 도달 불가한
죽은 코드다. **성능 후보가 아니라 문서·정합 항목**으로 강등한다. Σ 축 재도전은 §S13.46
결론대로 "다른 소비 지점"이거나 §S16.5의 HAR·어닝 이벤트 항처럼 **새 정보를 대각에 넣는**
방향으로만 가능하며, 추정기 수축은 이 제약집합에서 소진되었다.

**인벤토리 불변**(arm 미실행, 468).

---

## S16.5 판정 보류 — T1-3 risk_aversion 단위 정합 (사전등록 미완 · 새 사전점검 필요) — 2026-08-31

**실측 사실(확정)**: MVO objective `μ·w − λ·aᵀΣa − 0.03·‖Δw‖₁`에서 μ는 횡단면 z-score,
`aᵀΣa`는 일간 분산이다. 2026-08-18 리밸 실측:

| 항 | 규모 |
|---|---:|
| 리턴항 `μ·a` | 0.322 (최근 12 리밸 중앙값 0.338) |
| 리스크항 상한 `λ·aᵀΣa` (λ=1.0) | 4.86e-06 (= 0.035²/252) |
| 비율 | **약 18만배** |
| turnover항 `0.03·‖Δw‖₁` | 0.0024 (리턴항의 0.3%) |

즉 **`risk_aversion: 1.0`의 리스크 항은 목적함수에 사실상 존재하지 않으며, 리스크 통제는
전부 하드 TE 캡이 담당한다.** §S13.50이 관측한 "투영 변위의 80%가 제약 강제 유일해"의
구조적 원인이 이것이다.

**그러나 이것이 곧 결함이라는 결론은 성립하지 않는다** — 착수 전 재평가가 필요하다:
1. **하드 TE 캡이 이미 리스크 예산을 표현한다.** "리스크 예산 하에서 알파 최대화"는
   벤치마크 상대 운용의 표준 정식이며, 캡이 바인딩하는 26.8%의 리밸에서 λ는 제약의
   라그랑주 승수와 **중복**이다(캡 표면 위에서 μ·a를 최대화하는 것이 곧 평균-분산 효율해).
2. 캡이 바인딩하지 않는 73.2%에서 λ>0는 TE를 캡 아래로 더 끌어내린다 — 이는 개선이 아니라
   **리스크 예산 미사용**이며 기대 액티브 수익을 줄인다.
3. §S16.4가 Σ의 보정 무편향을 실증했으므로, **사용자가 제시한 순서 근거("T1-2로 Σ가
   신뢰할 만해진 다음")는 수정으로가 아니라 측정으로 충족되었다.**

**남는 검정 가능한 가설**: λ>0는 리스크 통제가 아니라 **μ 추정오차에 대한 정칙자**로
작동한다 — λ=0이면 옵티마이저가 노이즈가 섞인 μ가 허용하는 가장 극단적인 제약 코너로
몰리고(§S13.50의 코너 해 관측), λ>0는 포지션을 분산시킨다.

**λ*의 값이 데이터로 결정되지 않는다는 문제**: 세 가지 도출이 모두 다른 값을 준다 —
STX 단일 종목 기준 ≈ 26,000 / 활성 241종 median|(Σa)ᵢ| 기준 ≈ 76,000 /
두 항을 같게 하는 `(μ·a)/(aᵀΣa)` 기준 ≈ 105,000. **여러 값을 시험하는 것은 §2.4 스윕
금지 위반**이므로, 단일 사전등록 값을 정당화할 **새 사전점검**이 선행되어야 한다.

**착수 자격(사전등록 시 충족해야 할 게이트, 측정 전 고정)**:
- P1: 캡이 바인딩하지 **않는** 73.2% 리밸에서, 현 북이 제약집합 안에서 평균-분산
  비효율인지 — 같은 `aᵀΣa`로 더 높은 `μ·a`를 얻는 해가 존재하는지 §S13.50 E0 재구성
  자산으로 직접 측정. 존재하지 않으면 SHELVE(λ는 정의상 무익).
- P2: λ*를 자의성 없이 하나로 고정하는 규칙을 사전에 명시(예: "정칙자" 가설이면 λ를
  리스크 항이 turnover 항과 같아지는 값으로 — 두 정칙자의 등가 원칙).
- P3: §2.5 캐릭터 게이트 — 예상 TE·active share 이동을 사전 추정해 붕괴 여부를 먼저 판정.

**현 상태: arm 미구현·미실행. `risk_aversion` 기본값 1.0 무변경. 인벤토리 불변(468).**

---

## S16.2 사전등록 — 리비전 클리너 연장 상한 (측정 전 단독 커밋) — 2026-08-31

**대상**: §S15 fix-pack #5(2026-08-27 production 채택)가 만든 새 결함. `sellside.py:116-136`의
지속 연장 마스크에 **길이 상한도 아티팩트 판별 조건도 없어**, 실적발표 직후 컨센서스가
**진짜로** 붕괴해 중간 대역에 머무르면 그 기간 내내 붕괴 이전 값이 ffill로 복원된다.

**합성 재현 (production 파라미터 threshold 15 · extreme 50 · ratio 0.5, 검증자 3인 확인)**
- `80 → 10` 후 200일 지속: OFF는 t+1부터 10.0, **ON은 200일 전부 80.0으로 동결**
- `80 → 5` 후 39까지 실제 회복: 39 < 40이라 **끝까지 해제 없음**
- 해제 시 스텝이 threshold의 1.7~3.3배로 한 번에 유입(원시 diff 기준 마스크라 재정제 안 됨)

**소비처(왜 중요한가)**: 코어 피처 `eps_rev`·`eps_rev_ma_63d`·`eps_rev_trend`·
`sales_rev_ma_63d`, `mc_*_x_eps_rev` 3종, conditioning 브레드스, 그리고 **PEAD 오버레이의
`rev_quality`와 growth tilt**. 결과적으로 "리비전이 방금 무너진 종목"에 PEAD 부스트가
붙는다 — 오버레이 의도와 정반대. 기존 테스트는 30행·회복값 60 케이스만 검사해 미포착.

**단일 파라미터**: `revision_extension_max_days: 21` (기본 `None` = 현행 무제한 = 파리티).
**21BD의 근거(사전 고정)**: production `pead_max_days: 21`과 동일 — 연장이 이보다 길면
PEAD 오버레이 한 사이클 전체가 스테일 리비전을 소비한다. 추정치 롤오버 주기(63BD 분기)의
1/3. **스윕 금지**(§2.4) — 다른 값 시험 금지.

**비교 기준**: §S16.1 런(`outputs/s16_1_unit_fixpack`). arm variant =
`variants/s16_2_revision_extension_cap.yaml` = production + `s16_unit_fixpack_enabled: true`
(기준선에 포함) + `revision_extension_max_days: 21`(**유일한 델타**).

**채택 기준 = 정확성**(§S16.1과 같은 트랙). 다만 **새 파라미터가 생기므로 §S16.1에 묶지 않고
별도 사전등록**한다. ΔIR은 관측 기록. **인벤토리 비계수**(정확성 트랙).
- E0: `revision_extension_max_days=None` 경로 바이트 동일(단위 테스트 선행)
- E1(정확성 확증): `[RevisionClean]` 로그의 `extension-cells`·`extension-capped`가 0이 아닐 것
  (상한이 실제로 발동해야 측정 의미가 있음). 0이면 결함이 현 데이터에서 미발현이라는 뜻이므로
  **비액션으로 기록하고 채택하지 않는다.**
- E2(do-no-harm): TE ≤ 4.5% · active share 캐릭터 보존 · ECOS fallback 0 · turnover ≤ 1.25×

---

## S16.3 사전등록 — 퇴화 폴백을 고정 용량 신선 재적합으로 (측정 전 단독 커밋) — 2026-08-31

**대상(§S16 O1)**: 재훈련 모델의 트리 수가 `min_model_trees`(10) 미만이면
`walk_forward_train`이 **이전 모델과 그 피처 세트를 그대로 재사용**한다
(`model_trainer.py:793-799`). 인증 기준선 런의 33회 재훈련을 models 딕셔너리 **객체
동일성**으로 추적한 실측:
...
고유 모델 19/33 · 과거 최장 재사용 5회(2021-01-26 적합본 → 2022-01-13)
```

**게이트가 시간 축을 갖고 있지 않다**: `MAX_CONSECUTIVE_STALE_RETRAINS = 7`은 재훈련
**슬롯 수**만 세므로 `stale_depth = 4 ≤ 7`로 PASS한다. retrain_freq=63BD에서 한도 7은
약 1.75년의 모델 나이를 허용한다. 전체 퇴화율(14/33 = 42%)은 이미 report-only로 강등되어
있어 **어떤 게이트도 "모델이 1년 묵었다"를 잡지 못한다.**

**왜 이번엔 다른가**: §S10.2(min_child)·§S11.5-B(val_window)·§S11.10(lr)·§S13.8(단일 ES
지표)·§S13.47(eval_at)은 전부 "퇴화율을 낮추는" 하이퍼파라미터/지표 변경이었고 전부 실패했다.
**폴백 정책 자체를 바꾼 arm은 0건**이며 §S13.8 결과절이 "용량은 명시적으로 규제"를 잔존
후보로 남겼다.

**단일 파라미터**: `degenerate_fallback_mode: fresh_fixed` (기본 `reuse_prev` = 파리티).
퇴화 시 이전 모델을 재사용하지 않고 같은 창·같은 피처로 **조기종료 없이 고정 용량 재적합**한다.
용량 `degenerate_fresh_trees = 67` = 인증 기준선 런의 고유 19모델 best_iteration **중앙값**
(12,14,15,20,28,30,48,50,58,**67**,76,106,114,129,131,132,149,177,253). **사전등록 단일값,
스윕 금지**(§2.4). variant override에 노출하지 않는다.

**비교 기준**: §S16.1 런. arm variant = `variants/s16_3_fresh_fixed.yaml` = production +
`s16_unit_fixpack_enabled: true` + `degenerate_fallback_mode: fresh_fixed`(**유일한 델타**).

**게이트에 시간 축 추가(동반, report-only)**: `model_quality`에 `unique_models`·
`live_model_fit_date`·`live_model_age_retrains`를 기록하고,
`validate_portfolio_bundles.evaluate_production`에 `live_model_age_ok`
(`MAX_LIVE_MODEL_AGE_RETRAINS = 3` ≈ 189BD ≈ 9개월) 체크를 추가한다. 기존 체크·임계는 불변.

**판정**
- E0: `reuse_prev` 경로 바이트 동일(단위 테스트 선행)
- E1: **ΔIR > +0.36 & 서브기간 부호 일관**(§2.4 IR 채택 바). **병기 의무** =
  `unique_models`(19 → ?) · `live_model_age_retrains`(4 → 0 기대) · 퇴화율
- E2(do-no-harm): TE ≤ 4.5% · active share 캐릭터 보존 · turnover ≤ 1.25× · ECOS fallback 0
- **인벤토리 계수 대상**(성능 arm — 468 → 469)
- 상호배타 선언: "조기종료 제거 + 고정 용량"(Tier 2 T2-2 계열)과 같은 축이므로, 둘 중 하나만
  실행한다. 본 arm이 먼저이며 다중성 2를 사전 선언한다.

---

## S16.1 측정 결과 — 정확성 4항목 전부 확증, ΔIR +0.160 (2026-08-31)

**런**: `outputs/s16_1_unit_fixpack` · exit=0 · 97 리밸 · ECOS 194 · fallback 0
**빈티지**(신규 `data_vintage` 지문으로 산출물에서 직접 확인, §S16 O7):
workbook 2026-08-25T11:47:55Z(= KST 20:47:55) / Index 2026-08-25T07:37:36Z(= KST 16:37:36)
— 비교 기준 S0′와 **동일 빈티지 확인**.

**성과 관측(채택 기준이 아님 — 정확성 트랙)**

| 지표 | S0′ 1.5356 | S16.1 | Δ |
|---|---:|---:|---:|
| information_ratio | 1.535639 | **1.695332** | **+0.159693** |
| tracking_error | 0.037267 | 0.036997 | −0.000270 |
| realized_beta | 1.051505 | 1.050998 | −0.000507 |
| avg_ic | 0.019378 | 0.016038 | **−0.003340 (−17.2%)** |
| avg_annual_turnover | 0.697530 | 0.673669 | −0.023861 |
| active_share (Pictet) | 0.199390 | 0.200175 | +0.000785 |
| annual_return | 0.259849 | 0.266408 | +0.006559 |
| sharpe_ratio | 1.325404 | 1.359815 | +0.034411 |
| max_drawdown | −0.324554 | −0.327997 | −0.003443 |
| 퇴화 모델 | 14/33 (0.4242) | **12/33 (0.3636)** | −2 |

**avg_ic가 −17.2%인데 IR은 +0.16이다.** 이는 §S15(구조 경로만 수정 → avg_ic 비트 불변)와
달리 알파 경로를 직접 고쳤기 때문이며, **사전등록이 예측한 그대로다**(그래서 E1을 avg_ic
불변이 아니라 정확성 확증으로 재정의했다). §S13.12 프레임의 또 다른 사례 — 이번에는
유리한 방향이다: 통화·주식수 아티팩트를 제거하니 단변량 IC는 내려갔지만 북 전달은 올랐다.

**E1 (정확성 확증) — 사전등록 4항목**

| # | 기준 | S0′ | S16.1 | 판정 |
|---|---|---|---|---|
| 1 | `tg_upside` LN 8종이 −3.8σ 상수에서 벗어나 비-LN 분포와 겹칠 것 | 중앙값 −3.880, 범위 [−3.893, −3.861], 비-LN 5~95% 구간 진입 **0/8** | 중앙값 **+0.149**, 범위 [−0.859, +1.725], 진입 **4/8** | **PASS** |
| 2 | `best_calculated_fcf_level_z` KR·JP 제외 std ≥ 0.5 | **0.0008** | **0.9678** | **PASS** |
| 3 | `cash_conversion_z` 상위 6종이 발행주식수 순서와 불일치 | 9432·NVDA·AAPL·7203·RR/·IBKR (주식수 순) | IBKR·LYV·VRTX·PANW·WELL·GRMN | **PASS** |
| 4 | `\|corr(mc_rate_x_eps_rev, eps_rev)\| < 0.999` | 1.0000 | 1.0000 | **기준 오설계 — 아래** |

**E1-4 자기정정(사전등록 기준의 결함)**: 이 기준은 **판별력이 없다**. 상관계수는 스케일
불변이므로 날짜 내 상수 `c`에 대해 `corr(c·x, x) = sign(c)·1`이며, 외부 z-score를 건너뛰든
말든 항상 ±1이다. 수정이 바꾸는 것은 상관이 아니라 **진폭**이다. 사전등록 시 이 점을
놓쳤음을 기록한다.

올바른 통계량(**날짜별 횡단면 std의 분포** — 외부 z를 건너뛰면 std가 1.0 고정에서
`|매크로 스칼라|`에 비례해 변동해야 한다)으로 재측정:

| 피처 | S0′ std 중앙값 / 그 변동폭 / 5~95% | S16.1 std 중앙값 / 그 변동폭 / 5~95% |
|---|---|---|
| mc_rate_x_eps_rev | 0.964 / **0.081** / [0.923, 1.000] | 1.066 / **0.701** / [0.116, 2.362] |
| mc_slope_x_eps_rev | 0.964 / **0.025** / [0.923, 1.000] | 1.082 / **0.706** / [0.122, 2.385] |
| mc_dxy_x_eps_rev | 0.964 / **0.081** / [0.923, 1.000] | 1.093 / **0.683** / [0.095, 2.327] |
| mc_vix_x_mom252 | 0.945 / 0.254 / [0.000, 0.987] | 0.771 / 0.623 / [0.000, 2.007] |
| mc_vol_x_mom63 (대조) | 0.765 / 0.169 / [0.468, 0.940] | 1.022 / 0.172 / [0.801, 1.183] |

수정 전에는 날짜별 std가 z-score의 정의대로 1.0 근처에 못박혀 **매크로 크기가 소거**돼
있었고, 수정 후에는 0.1~2.4로 변동한다 — **매크로 진폭이 비로소 모델에 도달한다.**
두 레그가 모두 횡단면 변수인 대조군 `mc_vol_x_mom63`은 변동폭이 0.169 → 0.172로 사실상
불변이다(이 피처는 애초에 이중 z의 피해자가 아니었다는 사전 진단과 일치). **판정: PASS.**

**E2 (do-no-harm) — 전항 통과**: TE 3.700% ≤ 4.5% · Pictet active share 20.02%
(19.9% ± 3%p 안) · ECOS 194 / fallback 0 · turnover 0.674/0.698 = **0.966× ≤ 1.25×**.

**관측 계측(사전 예측 대조)**
- P4 클립 저촉률: 런 로그 `[MacroCross] skip-CS-z ON: pre-clip |z|>5 cells = 0.78%`
  — **사전 추정 0.6~0.8%와 일치**.
- 신규 `model_quality` 필드: `unique_models = 21` · `live_model_fit_date = 2025-08-28`
  · `live_model_age_retrains = 4` — §S16 O1의 라이브 모델 354일 스테일이 이제
  **산출물에서 직접 확인**된다(이전에는 pkl을 뜯어야 알 수 있었다).
- 부수 관측: `[RevisionClean] extension-cells = 133,443` — §S15 fix-pack #5의 연장이
  리비전 패널(3283×250)의 **16.3%**를 덮고 있다. §S16.2가 겨냥하는 결함이 희귀 사례가
  아님을 실증한다.

**판정: 정확성 근거 채택 권고.** 4개 결함 모두 실측으로 확인되었고 수정 후 의도한 정보가
복원되었음이 E1으로 확증되었다. ΔIR +0.160은 관측 기록이며 채택 사유가 아니다
(§S13.33·§S15 선례). **flip은 §8 사용자 결정.** 채택 시 **IR 1.6953이 새 S0′**가 되고
1.5356은 은퇴한다. 인벤토리 468 불변(선택 이벤트 아님).

**운영 사고 기록**: 최초 측정 시도(12:14 시작)는 Phase 5-6 중 `0xC000013A`
(콘솔 컨트롤 이벤트)로 사망했다 — §S13.27과 동일 실패 모드. PowerShell 래퍼
(`outputs/run_variant_task.ps1`, ES_SYSTEM_REQUIRED 유지)를 §S15.2에서 실제로 완주했던
**`.bat` + `cmd /c` 형식**(`outputs/s16_1_run.bat`)으로 바꾸자 exit=0으로 완주했다.
**장기 런은 .bat 형식이 검증된 패턴이며 PowerShell -File 래퍼는 회피한다.**

### S16.1 부록 — 구현 유닛 보고와의 교차검증 (2026-08-31, 비액션)

구현 유닛(독립 컨텍스트)이 메인과 **별개로** 도달한 결론·수치를 대조한다.

1. **P4 기준 오설계를 독립 발견.** 구현 유닛도 "날짜 내에서 `c·x`와 `x`는 두 번째
   z-score 여부와 무관하게 완전상관"임을 지적하고, 대안 시그니처로 **날짜별 횡단면 std**를
   제시했다. 합성 엔드투엔드 픽스처 실측 **OFF 정확히 1.0000 vs ON 0.489~2.639**.
   메인이 production 패널에서 측정한 값(std 변동폭 0.081 → 0.701, 5~95%
   [0.923,1.000] → [0.116,2.362])과 **서로 독립인 두 경로에서 같은 결론**이다.

2. **클립 저촉률 3자 대조.** 메인 사전 추정(매크로 스칼라 분포 실측 + 레그 정규 근사)
   0.6~0.8% / 구현 유닛의 부분 재구성 0.053%(`mc_slope_x_eps_rev` 단독,
   `fac_yield_slope` 피처를 스칼라 프록시로 사용 — 원시 스칼라가 코어 패널에 없어 4개 중
   1개만 재구성 가능) / **런 계측 실측 0.78%**(전 mc_* 합산). 계측값이 정본이며 메인의
   사전 추정과 일치한다. 유닛의 0.053%는 프록시·단일 항 한계에 의한 과소이며 방법 차이로
   설명된다.

3. **테스트 파일 배치**: P2/P3 테스트가 `tests/test_s16_unit_fixpack.py`가 아니라
   `tests/test_accounting.py`에 있는 이유는 TDD 훅이 `src/features/accounting.py` 편집 시
   모듈명 테스트 파일을 요구하기 때문이다(§S15의 `tests/test_factor.py` 선례). 신규
   테스트 총 14건(9 + 5).

4. **합격기준 설계 결함(메인 책임)**: 유닛에 준 AC 중 "타 파일 무변경"류는 **격리된
   트리를 전제**했으나 4개 유닛이 동일 워킹트리에서 동시 작업했다. 두 유닛이 이를
   FAIL로 정직하게 보고했고, 메인이 **파일 단위 귀속**으로 재판정해 각자 자기 레인만
   건드렸음을 확인한 뒤 유닛별로 분리 커밋했다. 향후 동시 유닛에는 worktree 격리를 쓰거나
   AC를 "내 담당 파일만"으로 한정해야 한다.

5. **미해결 관측**: 11:31~12:07 사이 워크북을 적재한 파이프라인 프로세스(PID 39044,
   CPU 29분, 1.2GB, `outputs/progress.md`에 Phase 2~4 기록)가 존재했고 메인이 종료시켰다.
   담당 유닛이 특정되지 않았다(해당 유닛은 자기 것이 아니라고 보고). 산출물 손상은 없었고
   `progress.md`는 복원했다. **동시 유닛 실행 시 워크북 적재 금지 지시의 실효성 확인
   수단이 없다**는 점만 기록한다.

---

## S16.2 측정 결과 — 연장 상한 21BD, 정확성 확증·ΔIR 노이즈 (2026-08-31)

**런**: `outputs/s16_2_revision_extension_cap` · 97 리밸 · ECOS 194 · fallback 0
**빈티지**: `data_vintage` 지문이 S16.1과 **비트 동일**(2026-08-25T11:47:55Z /
2026-08-25T07:37:36Z) — 동일 빈티지 페어 확인.
**비교 기준**: §S16.1 런(IR 1.695332), 유일한 델타 `revision_extension_max_days: 21`.

**E1 (정확성 확증) — PASS**

`[RevisionClean]` 계측 (상한 없음 → 21BD):

| 시트 | extension-cells (상한 없음) | extension-cells (21BD) | 감소 | extension-capped |
|---|---:|---:|---:|---:|
| Factset_EPS_Revision | 133,443 | **52,972** | **−60.3%** | **1,181** |
| Factset_Sales_Revision | 140,881 | **44,034** | **−68.7%** | **1,145** |

상한이 **2,326건의 이벤트에서 발동**했고 동결 셀의 60~69%가 해제되었다. 사전등록 E1
("extension-capped ≠ 0")을 충족한다. 더 중요한 것은 **규모**다 — §S15 fix-pack #5의 연장은
리비전 패널(3283×250 = 820,750셀)의 **16.3%**를 덮고 있었고, 그중 3분의 2가 21BD를 넘는
장기 동결이었다. 이 결함은 희귀 사례가 아니었다.

**성과 관측(채택 기준 아님)**

| 지표 | S16.1 | S16.2 | Δ |
|---|---:|---:|---:|
| information_ratio | 1.695332 | 1.714934 | **+0.019603** (노이즈 대역, \|ΔIR\| < 0.36) |
| avg_ic | 0.016038 | **0.018181** | **+0.002143 (+13.4%)** |
| tracking_error | 0.036997 | 0.037296 | +0.000299 |
| realized_beta | 1.050998 | 1.052419 | +0.001421 |
| avg_annual_turnover | 0.673669 | 0.703141 | +0.029472 (1.044×) |
| active_share | 0.200175 | 0.200073 | −0.000101 |
| max_drawdown | −0.327997 | −0.324675 | +0.003322 |
| 퇴화 모델 | 12/33 | 14/33 | +2 |

**avg_ic +13.4%가 기전의 직접 증거다**: 동결을 풀자 리비전 스트림이 신선해졌고 예측력이
올랐다. IR은 +0.020으로 노이즈지만, 이 트랙의 채택 사유는 IR이 아니라 정확성이다.

**E2 (do-no-harm) — 전항 통과**: TE 3.730% ≤ 4.5% · active share 20.01%(19.9% ± 3%p 안)
· turnover 1.044× ≤ 1.25× · ECOS 194 / fallback 0.

**판정: 정확성 근거 채택 권고.** flip은 §8 사용자 결정. 인벤토리 468 불변.

**출처 주의**: 이 런의 stdout 로그는 비어 있다(아래 운영 기록의 콘솔 분리 부작용). 위
`[RevisionClean]` 수치는 **동일 config·동일 빈티지의 직전 시도(13:16 런)** 로그에서 온
것이며, 클리너는 결정론적이므로 값이 같다. 독립 확증으로 `avg_ic`가 +13.4% 이동한 사실을
병기한다(리비전 스트림이 실제로 바뀌었다는 산출물 기반 증거).

**운영 기록 — 장기 런 사망 모드와 해소**
- S16.1 1차(12:14, `powershell -File` 래퍼 + ES_SYSTEM_REQUIRED)와 S16.2 1차(13:16,
  `.bat` 직접 실행) 모두 **Phase 5-6 중 `0xC000013A`(STATUS_CONTROL_C_EXIT)** 로 사망.
  §S13.27이 기록한 동일 코드다. 즉 **`.bat` 형식도, 슬립 억제도 단독으로는 불충분**하다.
- 해소: `.bat` 안에서 `start "title" /WAIT /MIN`으로 python에 **독립 콘솔**을 주면
  태스크 콘솔에 전달된 컨트롤 이벤트가 런으로 전파되지 않는다. 이 형태로 S16.2가 완주했다.
- 부작용: `start ... > log` 형태는 리다이렉트가 `start` 자신에 묶여 **로그가 빈다**.
  S16.3용 배치는 `start ... cmd /c "... > log 2>&1"`으로 리다이렉트를 안쪽에 넣어 해소했다.
- **권고 패턴(향후 장기 런)**: schtasks(Interactive·배터리 허용·StopOnIdleEnd=false)
  + `.bat` + `start /WAIT /MIN cmd /c "<명령> > <로그> 2>&1"`.

---

## S16.3 측정 결과 — fresh_fixed 기전 완전 작동·ΔIR +0.016 노이즈, E1 FAIL 불채택 (2026-08-31)

**런**: `outputs/s16_3_fresh_fixed` · exit=0 · 97 리밸 · ECOS 194 / fallback 0 · 1,105초
**빈티지**: `data_vintage` 지문이 §S16.1 런과 **비트 동일**(workbook 2026-08-25T11:47:55Z /
Index 2026-08-25T07:37:36Z) — 동일 빈티지 페어 확인.
**비교 기준**: §S16.1 런(IR 1.695332), 유일한 델타 `degenerate_fallback_mode: fresh_fixed`.
**판정 스크립트**: `scripts/eval_s16_3_arm.py`(+ 순수 헬퍼 테스트 4건) →
`outputs/s16_3_fresh_fixed/e1_summary.json`.

**E1 (ΔIR > +0.36 & 서브기간 부호 일관) — FAIL**

| 구간 | IR (S16.1) | IR (S16.3) | Δ |
|---|---:|---:|---:|
| full | 1.6953 | 1.7117 | **+0.0164** (bar +0.36 미달) |
| 분할 1 (2018-11~2021-06) | 1.6628 | 1.6857 | +0.0229 |
| 분할 2 (2021-06~2024-01) | 1.5416 | 1.5445 | +0.0029 |
| 분할 3 (2024-01~2026-08) | 1.7751 | 1.7934 | +0.0184 |

3분할 부호는 3/3 양(방향 일관)이나 크기가 노이즈 대역(|ΔIR| < 0.36)이다. §2.4에 따라
**IR 근거 채택 불가 — 불채택, default `reuse_prev` 유지.**

**병기 의무(사전등록) — 기전은 완전히 작동했다**

| 항목 | S16.1 (reuse_prev) | S16.3 (fresh_fixed) |
|---|---|---|
| fresh_fixed 발동 | — | **12회 = 퇴화 12건 전부** |
| unique_models | 21/33 | **33/33** |
| live_model_fit_date | 2025-08-28 (나이 354일) | **2026-08-17 (최신)** |
| live_model_age_retrains | 4 | **0** |
| 퇴화 판정 수 | 12/33 | 12/33 (동일 — 폴백은 탐지가 아니라 대응만 변경, 예상대로) |
| avg_ic | 0.016038 | **0.018295 (+14.1%)** |

**E2 (do-no-harm) — 전항 통과**: TE 3.718% ≤ 4.5% · active share 19.99%(base ± 3%p 안)
· turnover 0.671/0.674 = 0.996× ≤ 1.25× · ECOS 194 / fallback 0. beta 1.0523 ·
MaxDD −0.328(불변) · annual return 26.74% · sharpe 1.363.

**해석 — 이 arm이 산 것**: §S16 O1의 가설("라이브 모델이 354일 묵어 IR을 잃고 있다")은
**반증**되었다. 퇴화 슬롯 12개를 전부 신선 모델로 갈아끼우자 IC는 +14.1% 올랐지만 IR 전달은
+0.016 노이즈다 — `reuse_prev` 폴백은 사실상 **무해한 정책**이었다(§S13.12 전달 상한의 또
한 사례, 이번에는 "스테일 비용이 없다" 방향). 모델 나이는 성능 결함이 아니라 **운영 리스크**
이며, 이제 report-only 게이트(`live_model_age_ok`, 한도 3 retrains)로 산출물에서 상시
가시화된다(§S16.3 동반 계측, 기존 체크·임계 불변, default 무변경).

**축 종결**: §S13.8이 남긴 잔존 후보 "용량은 명시적으로 규제"가 이 arm으로 소진되었다.
사전등록의 상호배타 선언(다중성 2)에 따라 같은 축의 "조기종료 제거 + 고정 용량"(Tier 2
T2-2 계열)도 **실행 없이 종결**한다.

**인벤토리**: 468 → **469** (`experiment_inventory.json` 갱신 — 성능 arm 계수. S16.1·S16.2는
정확성 트랙이라 비계수, §S15 선례).

**운영 기록**: 1차 시도(14:15:33, schtasks)는 Phase 1 중 사망 — 원인은 **머신 재부팅**
(부팅 시각 14:41:58 실측, `0xC000013A`). §S16.2 운영 기록의 "패턴 불충분" 목록에 넣지
않는다(전원 이벤트는 어떤 실행 패턴도 못 살린다). 동일 태스크 재기동(15:20:59)으로
15:39:33 완주 — schtasks + `.bat` + `start /WAIT /MIN cmd /c` 패턴 자체는 유효 재확인.

---

## Production flip — S16.1 unit fix-pack 채택 (사용자 승인, 2026-08-31)

**결정**: 사용자가 §S16.1·§S16.2 flip을 승인("flip 승인할게"). §8 규칙대로 한 번에 1개씩 —
본 절은 S16.1, 다음 절이 S16.2.

**변경**: `variants/codex_causal_rank_65.yaml`에 `s16_unit_fixpack_enabled: true` 1줄
(+근거 주석). `PipelineConfig` 기본은 OFF 유지. **롤백 = 플래그 1줄 삭제**(E0 단위테스트가
바이트 동일 복원 인증).

**새 기준선**: **S0′ = IR 1.6953 / TE 3.700% / beta 1.0510 / avg_ic 0.016038 /
turnover 0.674 / 퇴화 12/33** (`outputs/s16_1_unit_fixpack`, 08-25 빈티지).
**1.5356은 은퇴 — arm 비교에 혼용 금지.**

**§8 체크리스트 이행(같은 커밋)**: ① variant 플래그+주석 ② acceptance
`post_arm_production_flags` 5개 파일 + `test_residual_sleeve.py` 인라인 allowlist 갱신
③ production 핀 테스트 신설(`test_production_variant_pins_s16_1_flip_state`)
④ 전체 스위트 708 PASS ⑤ 본 절.

**DSR**: 정확성 트랙(선택 이벤트 아님)이라 비계수·해킷 불요 — §S13.33·§S15 fix-pack 선례.

**flip 승인 시 이득 해부(사용자 질의로 추가 실측, 채택 근거 아님)**: ΔIR +0.160의 3분할은
**−0.014 / +0.554 / −0.027 (부호 비일관)**, 연도별로는 2023 +1.22 · 2022 +0.23 · 2026 +0.34에
집중이고 2020 −0.24 · 2021 −0.20. 즉 성능 arm 기준이었다면 FAIL 패턴이다(채택 근거가
정확성인 이유). 다만 집중 구간(2022~23)이 금리·달러·VIX 레짐기와 일치 — 4수정 중 가장
광범위한 P4(매크로 진폭 복원)가 가치를 갖는 바로 그 기간이라 기전-기간 정합은 성립한다.
북 실측: 평균 일간 L1 이동 0.231, 상위 무버 SLB +0.77%p(2022 FCF 강세 에너지 — P2 fcf_level_z
...
### §S16.7 측정 결과 (2026-09-01) — 전 게이트 통과, flip 후보 사용자 상신

**실행**: `outputs/s16_7_name_risk_cap`(빈티지 08-25 쌍 = S0′와 동일 확인). wall
46,458s는 야간 절전 동결 포함 — schtasks 패턴으로 생존, 재해 비용 반영 실 페이스는
리밸당 ~5-7s. 판정 스크립트 `scripts/eval_s16_7_arm.py`(테스트 4건 선행),
산출 `outputs/s16_7_name_risk_cap/e1_summary.json`.

| 게이트 | 결과 | 상세 |
|---|---|---|
| **G0** | **PASS** | data_vintage 쌍 동일 · avg_ic **비트 동일**(0.01818093285522787) — 옵티마이저-전용 변경 증명 |
| **G1** | **PASS** | arm 준수율 **100%(breach 0/97)** · 라이브 북(08-18) STX 몫 **0.5428 → 0.3439** · arm 최악값 0.3564(≤0.36). base 참조 breach 31/97(32%), 최악 0.7839(2024-11-08) |
| **G2** | **PASS** | TE 3.71%(≤4.5) · active share 20.01%(동일) · turnover **0.977×(감소)** · ECOS 194/fallback 0 · 퇴화 14/33 동일 · beta 1.0551 · MaxDD −0.3228(개선) |
| **E1** | 형식 FAIL | ΔIR **+0.0446**(1.7149→1.7596) < +0.36 바. 단 3분할 **+0.0634/+0.0351/+0.0267 전부 양**(부호 일관) — 노이즈 밴드 내 양수 |

**해석**: §S13.27-B(vol cap: 방어 실재하나 ΔIR −0.62)와 정반대 — name-share 캡은
**완전 바인딩(31→0 breach)하면서 IR 무비용**(오히려 노이즈 밴드 내 +0.045, 3분할 전부
양). 기전: 08-18 북에서 STX −0.82%p(3.73→2.90%)가 **동일 신호 군집 내 재배분**(MU
+0.56, VRT +0.81, 285A +0.28, 000660 +0.28, ACN +0.39)으로 흡수 — 알파 축은 보존하고
단일 종목 집중만 해소(§S13.41 선별 재배분과 같은 형태). turnover까지 감소한 것은 극단
포지션 빌드업/언와인드 왕복이 줄었기 때문으로 해석(가설 수준).

**판정**: 사전등록 채택 프레임(G1 바인딩 + G2 무손상 + ΔIR > −0.36) **충족** →
**flip 후보로 사용자 상신**(§8 체크리스트로 진행, 승인 대기). IR은 채택 근거가 아니라
...
### §S16.8 사전점검 결과 (2026-09-01) — A PROCEED · B SHELVE

`outputs/s16_8_precheck.json` (커버리지 3,011일, NW lag 20):

| | raw IC (NW t) | **잔차 IC (NW t)** | 최대 중복 |
|---|---:|---:|---|
| A `fwd_opcf_level_z` | 0.0280 (t 3.71) | **0.0244 (t 2.97)** | best_eps_chg_252d 0.568 |
| B `fwd_opcf_invest_divergence` | 0.0100 (t 1.50) | **0.0038 (t 0.47)** | best_sales_chg_252d 0.515 |

- **A PROCEED**(사용자 지시 재도전 + 참고로 게이트도 통과): 현재 executable score를
  잔차화하고도 IC 0.024가 남음 — 기존 62피처가 담지 못한 증분 실재. 중복 최대 0.57로
  경고선(0.8) 미달. **단위 수정이 OCF 레벨 신호를 실제로 살렸다는 정황**(수정 전
  §S13.4c는 yield 정식화로 ΔIR −0.180). 단, §S13.12 전달 상한(사전점검 IC → IR 전달
  ~9%) 전례상 arm 결과가 별개임을 병기.
- **B SHELVE(사전등록 게이트 발동, 실행 0·인벤토리 비계수)**: 발산 피처는 잔차 IC t
  0.47 — raw에서도 t 1.50에 그치고, 성분의 절반 이상이 이미 score에 기표현된 성장축
  (sales/eps 252d 성장과 ρ~0.5). **AI 선제투자 논리는 현 모델이 이미 OCF·FCF·CAPEX
  각각의 성장/레벨 축으로 흡수 중이라 발산 조합의 증분이 없음.** 재도전은 새 정식화
  사전등록 필요(사용자 오버라이드 가능).

### §S16.8 arm A 1차 런 무효 — Index.xlsx 빈티지 포크 사고 (2026-09-01)

arm A 1차 런(IR 1.582, wall 1426s)은 **판정 무효**: metrics의 `data_vintage`가
`fx_mtime 2026-09-01T02:15:42Z / size 0`을 기록 — DataLoader가 Index.xlsx의
**재작성 도중**(11:15:42 truncate 시작 → 11:19:11 완료, 일일 리프레시)에 vintage를
stat했고, 완료된 새 파일(86,441,553B, 09-01자 데이터 추가)을 읽었다. 기준선
S0′(1.7596)은 08-25 fx 빈티지이므로 **교차 빈티지 비교 금지**(§S13.47 제2 빈티지 축
규칙, §S13.46 FX 오염 재판정 전례). 1차 런의 ΔIR −0.178은 기록만 하고 폐기.

- 현 Index.xlsx 무결성 검증 PASS: FX 6페어 2014-01-01~2026-09-01, 최근 250d 결측 0.
  워크북(ai_signal_data.xlsx)은 08-25 그대로 — 포크는 fx 축 단독.
- **재판정 프로토콜**: 동일(09-01) 빈티지 쌍으로 ① S0 재인증
  런(`variants/s16_8_s0recert.yaml` = production config 사본, 재인증이므로 인벤토리
  비계수) → ② arm A 재실행, 순차 체인. 판정은 재인증 수치 기준(비트 동일하면 1.7596
  유지, 다르면 빈티지 포크 신 기준선 병기).
- **운영 규칙 신설**: 평일 ~11:15-11:30(일일 리프레시·운영 파이프라인 창)에는 백테스트
  기동 금지 + **기동 직전** (워크북, Index.xlsx) mtime 쌍 재확인(런 전 확인을 세션 초에
  한 번 하는 것으로는 불충분함이 실증됨).

### §S16.8 arm A 측정 결과 (2026-09-01, 동일 빈티지 재판정) — E1 FAIL 불채택

재판정 체인 완료: ① S0 재인증(`outputs/s16_8_s0recert`, 09-01 fx 빈티지) = **IR
1.75962 / TE 3.71% / avg_ic 비트 동일(0.018181) / 서브기간 1.523/1.582/2.028** —
S0′(1.7596)를 서브 정밀도까지 재현(**09-01 FX 리프레시는 소비 구간에 실질 무영향**,
기준선 1.7596 유효 유지). ② arm A 재실행 = 1차와 헤드라인·서브기간 동일(파이프라인
비트 결정성 재확인). 재인증·arm의 data_vintage 쌍 일치 확인. 판정
`outputs/s16_8a_opcf_level/e1_summary.json`(경량 판정 — name-cap 검사는 동일 북에서
기측정: 라이브 max share 0.285/top 6857/OK).

| 항목 | 결과 |
|---|---|
| **E1** | **FAIL** — ΔIR **−0.1775**(1.7596→1.5821), 3분할 **−0.325/−0.495/+0.248** 부호 비일관(손실 P1·P2 집중, 이득은 2024+만) |
| E2 | PASS 전 항목(TE 3.73%·active share 19.8%·turnover 0.992×·fallback 0·퇴화 14/33) |
| 피처 실효 | **no-op 아님** — EWMA 생존 33/33, 라이브 gain 2.07%(fcf_level_z 2.47%와 동급), avg_ic **+13.4%** |

**판정: 불채택, `fwd_opcf_level_feature_enabled` default-OFF 유지.** 사전점검 잔차 IC
NW t 2.97의 실재 증분 신호가 **IR 전달에서 음(−0.18)** — §S13.12 전달 상한·§S13.38
(IC +79%에도 ΔIR −0.276) 계열의 재확인 사례. 해석: OCF 레벨은 진짜 정보지만 모델이
이를 소비하며 기존 승자 조합(vol+FCF 축)의 랭킹을 P1·P2 구간에서 희석; 2024+에서만
순기여. **단위 수정 후에도 Fwd_OpCashflow 축은 레벨 정식화까지 소진 — 축 재종결.**
arm B(투자 발산)는 사전점검 SHELVE(실행 0) 유지. 인벤토리 470→471(A만 계수).

---

## S16.5-P — risk_aversion 사전점검 (사전등록, 2026-09-01, 사용자 지시 착수)

§S16.5(08-31) 보류 절의 착수 자격 게이트를 구체화한 read-only 사전점검. arm 없음 —
백테스트 0회, 오프라인 재해만. 표준 데이터원 = 새 기준선 pkl
(`outputs/s16_7_name_risk_cap`: 실행북·predictions) + 워크북(production cov 재구성,
eval_s16_7 방법론). 스크립트 `scripts/precheck_s16_5.py` →
`outputs/s16_5_precheck.json`.

**표본**: P0 진단은 97 리밸일 전수(재해 없음). 재해가 필요한 P1a/P1/P3는 **매 2번째
리밸(49일) 고정 표본**(짝수 인덱스, 사전 고정 — 선택 재량 0). 오프라인 재해 = production
config(캡 포함) 그대로 `optimize_portfolio`/`_build_mvo_constraints` 호출, 입력은
μ=executable predictions·Σ=production cov·prev=직전 실행북·bm=cap_weighted.

**측정·게이트(측정 전 고정)**:
- **P0 (현황 재계측, 비게이트)**: 리스크항/리턴항 비율 분포(≈1/18만 재확인), TE 캡
  바인딩률(실행북 ex-ante TE ≥ 0.995×3.5%), ex-ante TE 분포 — 08-31 수치(26.8%)의
  현 기준선 재측정.
- **P1a (관성 확정, 비게이트)**: 표본일 타깃 재해 w(λ=1) vs w(λ=0) — max‖Δw‖∞ 중앙값
  < 1e-6이면 "리스크 항 부재" 실측 확정.
- **P1 (효율성 — 결정 게이트)**: 표본일 중 캡 비바인딩일에서 등리스크 최대알파 해
  `max μ·a s.t. F ∧ {TE ≤ TE₀}` (F = production 제약 전체, turnover 하드 제약 포함,
  목적함수 페널티 없음)를 두 기준으로:
  ① TE₀ = 실행북 TE, 갭₁ = (μ·a* − μ·a_실행북)/|μ·a_실행북|;
  ② TE₀ = 타깃 TE, **갭₂ = (μ·a* − μ·a_타깃)/|μ·a_타깃|** — λ가 고칠 수 있는 것은
  타깃뿐이므로 **판정 기준 = 갭₂ 중앙값 ≥ +5%**. 갭₁은 실행 드래그 분해용 병기.
  갭₂ < 5%면 "타깃은 이미 제약 프런티어 위 — λ 정의상 무익" → **SHELVE**.
- **P2 (단일값 규칙, 조건부)**: §S16.5 로그의 정칙자 등가 원칙 채택 —
  **λ* = median_d[ 0.03·‖w_타깃−prev‖₁ / (a_타깃ᵀΣa_타깃) ]** (리스크 항이 turnover
  정칙자와 같은 크기가 되는 값). 참고 병기: 스케일 등가 median[(μ·a)/(aᵀΣa)]
  (08-31의 ≈105k 도출 재계산). λ 다중 시험 금지 — P3 재해는 λ* 1개만.
- **P3 (캐릭터 사전 게이트)**: 표본일을 config.risk_aversion=λ*로 재해 —
  ① Pictet active share 드리프트 전량 ±3%p 이내, ② ex-ante TE 중앙값 ≥ 0.5×(λ=1 재해)
  (무음 bm 붕괴 방지, 불변식 5), ③ 전 해 optimal(fallback 0), ④ 알파 보존율
  μ·a(λ*)/μ·a(λ=1) 병기(정보성).
- **판정 규칙**: P1 갭₂ ≥ 5% **AND** P3 ①②③ 통과 → arm(λ* 단일 사전등록) 상신,
  사용자 승인 후 실행. 어느 하나 미달 → **SHELVE**(risk_aversion 축 종결, 인벤토리
  비계수). "정칙자 가설"(λ가 μ 노이즈 코너를 완화)은 P1 갭₂가 0이어도 이론상 살아있으나,
  ex-ante 근거 없이 백테스트를 태우는 것은 §2.4 위반이므로 P1을 결정 게이트로 유지.

### §S16.5-P 측정 결과 (2026-09-01) — P1 FAIL → SHELVE, risk_aversion 축 종결

`outputs/s16_5_precheck.json` (97 리밸일 전수 P0 + 매 2번째 49일 재해, arm 0·백테스트 0):

| 게이트 | 결과 | 상세 |
|---|---|---|
| P0 | 재계측 | 리턴항/리스크항 중앙값 **114,931배**(08-31 "18만배"와 동일 차원), 실행북 TE 캡 바인딩률 24.7%(08-31 26.8% 정합), TE 중앙값 3.27% |
| P1a | 관성 확정 | w(λ=1) vs w(λ=0): max‖Δw‖∞ 중앙값 **4.1e-08**(최대 1.6e-06) — 리스크 항은 가중치에 사실상 0 기여 |
| **P1** | **FAIL** | **갭₂(타깃 대비 등리스크 최대알파) 중앙값 +0.024%** ≪ 바 5% (비바인딩 타깃 4일 — 타깃은 실행북과 달리 캡을 거의 항상 채움). 갭₁(실행북 대비)은 +33.3%(38일) — 전부 실행 경로(eta 0.5·no-trade band) 드래그로 λ가 접근 불가 |
| P2 | 단일값 | 정칙자 등가 λ* = **926**, 스케일 등가 = 125,976(08-31 ≈105k 도출과 정합) |
| P3 | PASS(무의미) | λ*=926 재해가 λ=1과 **완전 동일 북**(드리프트 0·TE비 1.0·알파 보존 1.0) — 926은 115k 스케일 대비 124배 부족해 아무 것도 안 움직임 |

**판정: SHELVE — risk_aversion 축 종결.** 결론 3줄:
1. **타깃은 이미 제약 프런티어 위에 있다**(같은 리스크로 얻을 추가 알파 0.02%): 하드 TE
   캡+종목/섹터 제약 집합이 리스크 예산을 완결적으로 표현하므로 λ는 잉여 — §S16.5의
   "결함 아님" 가설이 실측 확정.
2. 원리 있는 유일 규칙(정칙자 등가)의 λ*=926은 **증명된 no-op**, 실효 규모(~10⁵)의 λ는
   자의적 선택(스윕 금지)이며 P1상 ex-ante 손실만 가능. 정칙자(OOS) 가설은 ex-ante 근거
   0으로 백테스트 불가.
3. **부수 관찰(비액션, 별도 축)**: 갭₁ 33% — 실행북은 등리스크 달성가능 알파의 ~2/3만
   실현. eta 0.5·no-trade band의 의도된 turnover 경제학이며 λ와 무관. 실행 경로(eta)
...
### S17.1 사전점검 결과 ① — 피처 채널 정확성 (2026-09-02, 사전등록 커밋 be7b869 이후 실행)

`scripts/precheck_s17_feature_fixes.py` → `outputs/s17_prechecks/feature_fixes_accuracy.json`(로그 `.log`), 워크북
2026-09-01 14:05:40 빈티지.

**A(T-01) E1 정확성 — PASS 3/3**

| 항목 | OFF | ON | 게이트 |
|---|---:|---:|---|
| A1 VRT 갭창(2018-08-01~2020-02-24, 409행) tg_upside z = +5.0 클립 행 | **409** | — | ≥1 (재현) ✓ |
| A2 같은 창 ON NaN 행 | — | **409/409** | 전부 ✓ |
| A3 갭창 타 249종 z 횡단면 std 중앙값 | 0.433 | **0.974** | ≥ 0.90 ✓ |
| 갭창 타 종목 \|z\| p95 중앙값 (전 63BD 1.949 / 후 63BD 1.990) | 0.795 | 1.912 | 관측 — 2.4× 압축 해소 |
| tg_mom_63d VRT 갭 종료 후 63BD ≤ −4.99 비율 | 95.2% | **0.0%** | 관측 — 갭 종료 아티팩트 소멸 |

갭 셀 총 **693**(= 변경 셀 693, 정확히 커버리지 갭만 바뀜): VRT 409 · VST 127 · BE/PLTR/RACE/APP 16 · CRWD/ZS/DDOG 15
· TEAM/NET 14 · KEYS 8 · LITE 4 · HPE 3 · RDDT/SNOW 2 · 285A 1 — 보고서의 VRT/VST/PLTR/CRWD/LITE 외 12종이 추가로
드러남(모두 상장 후 FactSet 커버리지 개시 전 구간).

**C(M2 피처) E1 정확성 — PASS 2/2, 단 관측 1건이 사전 기대와 크게 어긋남**

| 지역(n) | beta_63d 중앙값 OFF → ON | 비 | 게이트 | idio_vol_63d OFF → ON (비) |
|---|---:|---:|---|---:|
| ASIA(16) | 0.282 → 0.773 | **2.74×** | ≥ 2.0 ✓ | 0.288 → 0.242 (0.84) |
| EU(43) | 0.767 → 0.841 | 1.10× | 관측 | 0.204 → 0.182 (0.89) |
| US(191) | 1.018 → 0.953 | 0.936× | \|·−1\| ≤ 0.15 ✓ | 0.210 → 0.197 (0.94) |

아시아 종목별 beta 중앙값: 000660 0.55→1.20 · 005930 0.41→0.89 · 8035 0.37→1.19 · 6146 0.39→1.20 · 7203 0.24→0.61
· 9432/9433 0.06→0.19 (통신 2종은 겹침 후에도 저베타) 등 16/16 상승. 유효 3,222일.

- **관측 ⚠ (기대 이탈)**: vol-quality 틸트(production ON) 의 상위 idio 터실 멤버십이 같은 날 OFF vs ON 에서 평균
  **25.5종/일 대칭차**(≈ 터실 83종 중 12.7종 교체, 15%) — 사전 기대는 아시아 비동기 몫 −0.8종/일. 원인은 편향이
  아니라 **분산**: 63일 창의 5일 겹침 합은 실효 비중첩 표본이 ≈12.6주라 idio 추정 상대 SE 가 일별(≈9%)의 약 2배
  (≈20%)로 커져 터실 경계 근처 이름이 뒤바뀐다. 전 지역 idio 0.84~0.94× 는 개별주 단기 음의 자기상관(주간 분산 <
  5×일별 분산)의 결과로 스케일 이동(횡단면 z 가 흡수). **판단**: B1/B2 게이트는 통과하므로 arm C 는 사전등록대로
  측정하되, 이 분산 증가가 E2(turnover·AS)·ΔIR 에 나타나면 채택 대신 **저분산 대안(Dimson lag-1 베타 + 일별 잔차
  idio, 보고서 G1-03 원안)** 을 새 사전등록으로 돌린다. 측정 전 명시.

### S17.1 사전점검 결과 ② — Σ 채널 기전 G1 **FAIL → arm B SHELVE(미실행)** (2026-09-02)

`scripts/precheck_s17_cov_overlap.py` → `outputs/s17_prechecks/cov_overlap_mechanism.json`(로그 `.log`). 동결 S0′ 97
리밸 실행 북 고정, production 경로(raw 리스크 원천 + S13.41 대각) Σ OFF/ON 재추정. ASIA 16 / EU 43 / US 191.

| G1 | 사전등록 정의 | 결과 | 판정 |
|---|---|---:|---|
| (i) ASIA×US 블록 평균 상관 ON/OFF ≥ 2.0 인 날짜 비율 ≥ 0.90 | 사전등록 산식 그대로(비 = on/off) | **0.753** | 형식 FAIL — 단 **산식 결함**: 19/97 날짜에서 OFF 블록 상관이 **음수**(2019-08~2020-02 7회, 2023-05~08 4회, 2025-05~09 6회, −0.001~−0.071)라 비가 정의되지 않고 NaN=FAIL 로 세어짐. 이 19일은 ON 이 전부 양(+0.05~+0.29)인 **가장 강한 회복 사례**. 정정 정의 on ≥ max(2·off, 0) ∧ on > off → **0.948(92/97)**, on > off 는 **97/97** → 정정 기준 PASS |
| (ii) US×US 블록 \|ON/OFF − 1\| 중앙값 < 0.10 | 그대로 | **0.1055** (사분위 0.065 / 0.105 / 0.195) | **FAIL** |
| (iii) ON Σ PSD 전 날짜 | 그대로 | True | PASS |

**관측(사전등록)**: 블록 중앙값 ASIA×US 0.0415 → 0.1517(**3.71×**, W3 3.1× 재확인·연도별 2.4~7.0×), EU×US 1.28×,
US×US 0.935×. ex-ante TE 중앙값 3.251% → 3.257% (**+0.8%**, 사분위 0.95~1.06), 아시아 액티브 분산 점유 12.4% →
14.7%, 아시아 순액티브 +1.7%, 조건수 1.34e6 → 1.86e6. **MZ 회귀(n 94, NW lag 3)**: β_d 1.60 (CI 0.32~2.88), β_w 0.88
(CI −0.08~1.85) — 사전 명시대로 검정력 없음. 실현 주간/일별 분산 비 중앙값 **1.00**, 실현/예측 분산 비 OFF 1.34 ·
ON 1.36(둘 다 같은 정도 과소 — 예측 부족분은 비동기와 무관한 공통 성분).

**판정 — SHELVE(사전등록 게이트 (ii) FAIL, arm 미실행)**. 해석: 전체 상관행렬을 5일 겹침으로 바꾸면 아시아 블록은
의도대로 회복되지만 **US×US 191종 블록(북 액티브 분산의 대부분)의 상관도 중앙 10.5%·상위 사분위 19.5% 움직인다** —
목표 효과(북 TE +0.8%)보다 큰 잡음 비용이며 arm 에서 ΔIR/TE 가 움직여도 비동기 수정 때문인지 US 상관 잡음
때문인지 분리할 수 없다. 실현 주간/일별 분산 비 1.00 은 **이 북이 비동기 편향으로 실제 리스크를 놓치고 있지 않음**을
보여준다(아시아 순액티브 +1.7% 규모에서 편향은 실재하나 북 수준에서는 비물질적). **후속 후보(새 사전등록 필요,
G5-01b)**: 아시아×비아시아 블록에만 lag-1 교차공분산(Scholes-Williams) 또는 겹침 상관을 넣고 US×US·EU×US 는 일별
유지 + PSD 복원 — 잡음 비용 없이 (i) 만 겨냥. **인벤토리 473 → 472**(사전점검 SHELVE·미실행 arm 은 비계수, §S16.8-B
선례). arm 체인은 A → C → D 로 축소 실행.

### S17.1 측정 결과 ① — arm A(T-01 커버리지 갭)·C(M2 β/idio 겹침) (2026-09-03, `scripts/eval_s17_arm.py`)

**실행 기록**: `outputs/s17_run_chain.bat`(schtasks `s17_chain`, 2026-09-02 23:02 기동) 직렬 A → C → D. A EXIT 0
(23:02→23:43, elapsed 2,468s) · C EXIT 0 (23:43→00:34, 3,028s). 두 런 모두 VINTAGE_PRE = VINTAGE_POST = (워크북
2026-09-01 14:05:40 / Index 2026-09-02 11:09:18), 판정 G0 `data_vintage` 동일 → 동결 S0′ `outputs/s17_s0_0902`(IR
1.7052) 와 직접 비교 가능. **D 는 00:34 기동 후 Phase 4 재학습 4회(2019-08-15) 시점인 01:06:43 에 시스템 재부팅(부팅
01:11:07)으로 소실** — 러너 결함 아님(EXIT 미기록·chain DONE 만 기록, 킬 로그는
`outputs/s17_4_dead_feature_prune_run.killed_0903_0106.log` 보존). 09-03 09:53 동일 빈티지 쌍에서 D 단독 재실행
(`outputs/s17_run_d_rerun.bat`, schtasks `s17_d_rerun`) → 결과 ②. 판정 산출물 `outputs/<label>/e1_summary.json`,
관측 진단 `outputs/s17_prechecks/ac_arm_observations.json`(IC 3분할·연도별, 패널 변경 셀, C 터실 churn).

| 항목 | S0′ (s17_s0_0902) | **A** s17_1_coverage_gap_fix | **C** s17_3_beta_overlap |
|---|---:|---:|---:|
| IR (metrics.json, full) | 1.7052 | 1.7330 (**ΔIR +0.0278**) | 1.8493 (**ΔIR +0.1441**) |
| 3분할 ΔIR (18-11~21-06 / 21-06~24-01 / 24-01~26-09) | 1.522 / 1.686 / 1.804 | +0.149 / **−0.175** / +0.089 | +0.006 / **−0.023** / **+0.392** |
| E1 형식(ΔIR > +0.36 ∧ 부호 일관) | — | FAIL — **관측(정확성 트랙, 채택 근거 아님)** | FAIL — 관측 |
| TE | 3.641% | 3.636% | 3.534% |
| turnover (비) | 0.6614 | 0.6588 (0.996×) | 0.6521 (0.986×) |
| Pictet active share | 20.02% | 20.21% (+0.19%p) | 20.31% (+0.29%p) |
| 실현 β | 1.050 | 1.048 | 1.059 |
| avg_ic (리밸일 IC, n 96) | 0.016306 | **0.019641 (+20.4%)** | **0.013826 (−15.2%)** |
| IC 3분할 Δ | 0.0104 / 0.0408 / −0.0022 | **+0.0044 / +0.0043 / +0.0013 (3/3 ↑)** | **−0.0004 / −0.0034 / −0.0036 (3/3 ↓)** |
| 퇴화 | 12/33 | 13/33 | 9/33 |
| MaxDD | −31.6% | −32.5% | −31.9% |
| ECOS / fallback | 194 / 0 | 194 / 0 | 194 / 0 |
| E2 do-no-harm (TE≤4.5%·AS±3%p·turnover≤1.25×·fallback 0) | — | **PASS 4/4** | **PASS 4/4** |
| 기전 | — | 패널 VRT tg_upside +5 클립 셀 **409 → 0** ✓ · 사전점검 A1~A3 ✓ | 사전점검 B1 2.74× · B2 0.936 ✓ |
| **flip 후보 (사전등록 정의 G0∧E2∧기전)** | — | **TRUE** | **TRUE(형식)** — 아래 관측 참조 |

**패널 변경 범위(변경 셀 수, 다른 피처 변경 0)**: A — `tg_upside` 158,972 · `tg_mom_63d` 158,797 (갭 셀 693 이 그 날짜의
횡단면 z 재표준화로 전 종목에 전파 — 사전점검 A3 의 std 0.43→0.97 회복과 같은 현상, 의도된 범위). C — `beta_63d`
806,335 · `idio_vol_63d` 789,416 (두 피처 전 셀).

**A 해석**: 정확성 수정(커버리지 갭 이전 셀 NaN→날짜별 median)이 알파 채널에 **일관되게 양(IC 3/3 분할 ↑, 연도별 7/9
↑)** 으로 전달되고, 리스크·실행 계량(TE·turnover·AS·β·fallback)은 불변. ΔIR +0.028 은 잡음대(부호 비일관)이며 채택
근거가 아님(§S16.1 선례 그대로). 부수 관측: 퇴화 12→13(잡음 범위). **§8 flip 후보 — 사용자 결정 대기.**

**C 해석 ⚠**: 정확성 게이트(B1·B2)와 E2 는 통과하나, 사전점검 ①에서 예고한 **분산 증가가 실제로 알파 채널에 나타났다**:
(i) 파이프라인 패널 기준 idio 상위 터실(90종) 일별 대칭차 **26.2종/일**(사전점검 25.5 재확인 — vol-quality 틸트 대상
집합의 15% 가 매일 뒤바뀜); (ii) avg_ic **−15.2%, 3/3 분할 전부 하락**(연도별 2023 0.082→0.060, 2024 0.013→0.005);
(iii) ΔIR +0.144 는 **3분할(24-01~26-09) +0.392 단독**이고 같은 구간 IC 는 하락 → 이득은 예측력 경로가 아니라
리스크/틸트 경로(TE −0.11%p·β +0.009, 아시아 β 회복에 따른 북 재배분)에서 오며 §S13.47 형 "이득 2024+ 집중" 패턴.
사전점검 ①의 사전 명시("분산 증가가 E2·ΔIR 에 나타나면 채택 대신 저분산 대안") 의 문자적 조건(E2·ΔIR)은 걸리지
않았지만 그 실체(추정 분산 2배)가 IC 로 확인되므로 **메인 권고: C 는 flip 보류, 저분산 대안(Dimson lag-1 β + 일별 잔차
idio, §S17 보고서 G1-03 원안)을 새 사전등록으로 재측정**. 형식상 flip 후보이므로 최종은 사용자 결정(§8) — 채택 시
S0′ 재수립 필수, A 채택 위에서 순차.

### S17.1 측정 결과 ② — arm D(M4 죽은 피처 prune, 재실행) + 종합 판정 (2026-09-03)

**실행 기록**: `outputs/s17_run_d_rerun.bat`(schtasks `s17_d_rerun`) 09:53:33 → EXIT 0 10:12:19, elapsed **1,117s**(재부팅
직후라 Phase 1 로드 7분; 전날 킬 런은 25분). VINTAGE_PRE = VINTAGE_POST = 동결 S0′ 쌍, G0 PASS. 재실행은 소실 런의
대체이며 추가 시행이 아니다(인벤토리 불변). 판정 `outputs/s17_4_dead_feature_prune/e1_summary.json`, 관측
`outputs/s17_prechecks/d_arm_observations.json`.

| 항목 | S0′ (s17_s0_0902) | **D** s17_4_dead_feature_prune |
|---|---:|---:|
| IR (full) | 1.7052 | 1.6996 (**ΔIR −0.0056**) |
| 3분할 ΔIR | 1.522 / 1.686 / 1.804 | **−0.242 / −0.262 / +0.448** |
| no-harm (ΔIR > −0.36 ∧ 3분할 전부 음 아님) | — | **PASS** (형식 E1 FAIL — 사전등록대로 성과로 읽지 않음) |
| TE | 3.641% | 3.765% (+0.12%p) |
| turnover (비) | 0.6614 | 0.7104 (**1.074×**) |
| Pictet active share | 20.02% | 19.54% (−0.49%p) |
| 실현 β | 1.050 | 1.057 |
| avg_ic | 0.016306 | 0.020279 (+24.4%) |
| IC 3분할 Δ | — | +0.0110 / +0.0034 / −0.0025 |
| 퇴화 | 12/33 | 10/33 |
| MaxDD | −31.6% | −32.7% |
| ECOS / fallback | 194 / 0 | 194 / 0 |
| E2 do-no-harm | — | **PASS 4/4** |
| 기전 | 모델 활성 피처 62·65 | **33/33 모델 정확히 56, 죽은 9개 0** ✓ · 패널 변경 셀 0(화이트리스트 단계 제외, 패널 자체는 불변) |
| **flip 후보 (G0∧E2∧기전∧no-harm)** | — | **TRUE** |

**해석**: (i) 기전은 완전 작동 — 모든 모델이 56 피처로 학습되고 죽은 9개는 어느 모델에도 없다. 사전 기록대로 **EWMA
선택층은 prune 후 inert**(기준 모델은 62/65 로 3개 드롭이 있었으나 arm 은 33/33 전부 56 = 드롭 0; 56 < `ewma_min_features`
60). (ii) ΔIR −0.006 은 사전등록대로 **시드 잡음**이며, 3분할 스윙(−0.24/−0.26/+0.45)이 이를 실증한다 — `colsample_bytree`
0.8 에서 트리당 기대 실피처 표본 수는 prune 전 52×56/65 = 44.8, 후 56×0.8 = 44.8 로 **동일**하므로 학습 용량 변화는
0 이고 RNG 실현만 바뀐다. 따라서 avg_ic +24% 도 같은 이유로 성과로 읽지 않는다(§S12.3(i) 관용). (iii) 비용: turnover
+7.4%·TE +0.12%p·AS −0.5%p 는 E2 안이지만 0 이 아니며, 역시 재실현된 모델의 북 차이라 위생 효과에 귀속할 수 없다.

**종합 판정 (S17.1, 사전등록 arm 4종 → 측정 3 + SHELVE 1)**

| arm | 프레임 | 판정 | 메인 권고 |
|---|---|---|---|
| A `s17_1_coverage_gap_fix` (T-01) | 정확성 | G0·E2·기전 PASS, IC +20%(3/3 ↑), 리스크 계량 불변 | **flip 권고** — §8 체크리스트, 채택 시 A 런(IR 1.7330·TE 3.64%·`outputs/s17_1_coverage_gap_fix`)이 동일 빈티지 새 S0′ |
| B `s17_2_cov_corr_overlap` (M2 Σ) | 리스크 규율 | 사전점검 G1(ii) FAIL → SHELVE(미실행) | G5-01b(아시아 블록 한정 보정) 새 사전등록 후보 |
| C `s17_3_beta_overlap` (M2 피처) | 정확성 | G0·E2·기전 PASS(형식 flip 후보)이나 IC −15%(3/3 ↓)·터실 churn 26종/일·이득 2024+ 단독 | **flip 보류 권고** — 저분산 대안(Dimson lag-1 β + 일별 잔차 idio, G1-03) 새 사전등록 |
| D `s17_4_dead_feature_prune` (M4) | 위생 | G0·E2·기전·no-harm PASS, ΔIR 시드 잡음 | **선택적 flip** — 위생 근거만(성과 기대 0); 채택 시 S0′ 재수립(IR 1.6996·turnover +7%)과 EWMA 선택층 영구 inert 를 수용해야 함. 대안은 OFF 유지 + 가드 테스트(`tests/test_s17_dead_feature_prune.py`, 이미 존재)만 |

flip 은 한 번에 1개(§8), 순서 후보 A → (D) . 각 flip 커밋은 §8 체크리스트 5항목(variant 1줄·핀 테스트 allowlist·production
핀·전체 pytest·결정 로그 flip 절)을 같은 커밋에서. **인벤토리 472 불변**(D 는 사전등록 시 계수, A·C 비계수, 재실행은
...
### S17.3 사전점검 결과 — 데이터 게이트 A~D **전부 PASS → arm 실행** (2026-09-03 17:00~17:07, 사전등록 커밋 489e55f 이후)

`scripts/precheck_s17_nominal_price.py` → `outputs/s17_prechecks/nominal_price_gates.json`, 워크북 2026-09-03 15:51:22 빈티지.
로더 진단: 명목 패널 250/250 종목, 비 UNADJ/PX_LAST 최소 0.996 · 중앙 1.076 · 최대 2.719, PE/PB/PEG 3시트 역산.

| 게이트 | 정의 | OFF(조정가) | **ON(명목가)** | 판정 |
|---|---|---:|---:|---|
| A | 무배당 5종 원시 UNADJ == PX_LAST | — | TSLA·AMZN·ADBE·NFLX·ISRG 4,628일 전부 max|Δ| **0.0** | PASS |
| B | MO 2014-06-30 UNADJ ≈ 41.94 | 19.5639 | **41.94** (Δ 0) | PASS |
| C | 고배당 12종 2014 median(TG/PX) ∈ [1.00, 1.25] | **2.103** | **1.084** | PASS |
| D | 고배당군 tg_upside_z 격차 (2014~19 − 2025~26) 절대값 < 0.5 | **+2.255** (학습기 +1.98 → 라이브 −0.27) | **+0.235** (−0.19 → −0.43) | PASS |

고배당 12종(데이터 정의, 2014-06-30 PX_LAST/UNADJ 비 최저): T 0.373 · RIO 0.415 · RTX 0.445 · MO 0.467 · TTE 0.496 · ZURN 0.501 ·
HSBA 0.514 · WMB 0.517 · CS 0.524 · VZ 0.527 · SHEL 0.535 · BNP 0.539. 연도별 TG/PX 중앙값 OFF 2.10(2014)→1.10(2026) 의 단조 하락
아티팩트가 ON 에서는 1.05~1.16 의 평탄한 띠로 바뀐다(§S17 보고서 "2.02→1.10 vs 무배당 1.14~1.31 평탄" 재현). 무배당군 z 격차도
OFF −1.22 → ON −0.35 로 축소(고배당군 팽창의 거울상이 사라짐). **관측**: MO 2014 내재 주식수(시총/가격) OFF 4,258M → ON
1,986M (**비 2.144**, 보고서 2.15 재현; 실제 발행주식 ≈ 1.98B), MO 2014-06-30 PE OFF 7.35 → ON 15.76(보고서 "시트 7.7 vs 실제 ~16"
재현), ON tg_upside |z| ≥ 4.99 셀 **0**.

**판정**: 4/4 PASS — 사전등록대로 arm `s17_5_nominal_price` 실행. 운영 사고: 09-03 17:07 schtasks 기동은 bat 시작 직후 러너가 뜨지
못하고 소실(Task Scheduler CIM 호출 취소·17:45 재시도 결과 0x800710E0 "요청 거부", 야간 절전 추정) → 09-04 09:58 `Start-Process`
로 직접 재기동(빈티지 쌍 15:51:22 / 14:26:46 불변 확인). 결과는 다음 절.

### S17.3 측정 결과 — arm `s17_5_nominal_price` (2026-09-04, `scripts/eval_s17_arm.py`, base = s17_2_s0recert)

**실행**: `outputs/s17_5_nominal_price_run.bat`(직접 기동) 09:59:23 → EXIT 0 10:37:13 (elapsed 2,143s). VINTAGE_PRE = POST = 동결 S0′ 쌍
(워크북 2026-09-03 15:51:22 / Index 2026-09-03 14:26:46) → **G0 PASS**. 판정 `outputs/s17_5_nominal_price/e1_summary.json`, 관측
`outputs/s17_prechecks/e_arm_observations.json`(IC 3분할·연도별, 패널 변경 피처, 모델 gain 점유).

| 항목 | S0′ (s17_2_s0recert) | **arm** s17_5_nominal_price | Δ |
|---|---:|---:|---:|
| IR (full) | 1.7118 | **1.7633** | **+0.0515** (관측, E1 형식 FAIL) |
| 3분할 ΔIR (18-11~21-06 / 21-06~24-01 / 24-02~26-09) | 1.503 / 1.676 / 1.853 | 1.359 / 1.725 / 2.062 | **−0.144** / +0.049 / **+0.209** |
| P4 tail IR (최근 구간) | 0.252 | 1.029 | +0.78 |
| TE | 3.676% | 3.643% | −0.03%p |
| turnover (비) | 0.6735 | 0.6813 (1.012×) | +1.2% |
| Pictet active share | 19.86% | 19.42% | −0.44%p |
| 실현 β | 1.052 | 1.054 | +0.003 |
| avg_ic (리밸일 IC, n 96) | 0.018935 | 0.017404 | **−8.1%** (3분할 −0.0062 / +0.0053 / −0.0037, 연도별 4↑/5↓) |
| 퇴화 | 10/33 | 10/33 | 동일 |
| MaxDD | −32.5% | −31.8% | +0.7%p |
| ECOS / fallback | 194 / 0 | 194 / 0 | — |
| E2 do-no-harm | — | **PASS 4/4** | |
| 기전 | — | 사전점검 4/4 PASS ∧ 패널 고배당군 2014 tg_upside 중앙값 **+2.20 → −0.08** | **PASS** |
| **flip 후보 (G0∧E2∧기전)** | — | **TRUE** | |

**패널 변경 범위(변경 피처 10, 전부 의도된 입력 계약 범위)**: `tg_upside` 820k · `cash_conversion_z` 817k · `fin_roe_pe_gap`/`fin_roe_pb_gap`
822k · `fin_pe_chg_63d`/`fin_pb_chg_63d` 803k · `fin_pe_level_z`/`fin_pb_level_z`/`best_px_bps_ratio_level_z`/`best_peg_ratio_level_z` 759k
셀 — 가격·수익률·모멘텀·Σ 피처 변경 0.

**기전 — 학습기 신호 복원(§S17 P1 예측 검증)**: 33 모델 gain 점유율 `tg_upside` 중앙 1.26% → 1.27% 로 같지만 **라이브 4모델은 0.0%
→ 1.27%** (§S17 "라이브 4개 모델 tg_upside gain 0.0" 이 명목가 분모로 해소 — 학습기가 다시 목표주가 괴리를 쓴다).
`cash_conversion_z` 1.50% → 1.10%(라이브 1.59 → 0.94: 주식수 2.1배 과대가 만들던 가짜 분산 소거). 이득은 최근 구간 집중(3분할
+0.21·P4 tail 0.25 → 1.03)이고 1분할은 −0.14 — 학습기 초반(2014~19 학습창)에서 고배당군 tg_upside 가 +1.98 z 의 인위적 순위를
잃으면서 그 시기 모델의 순위가 바뀐 결과로, avg_ic −8% 도 같은 구간(1분할 −0.006)에서 나온다. 부호 비일관 ΔIR 은 잡음대이며 정확성
트랙에서는 채택 근거가 아니다(§S16.1·§S17.1-A 선례).

**판정 — 정확성 트랙 flip 후보(G0·E2·기전 PASS)**. 메인 권고: **flip 권고** — 데이터 게이트 4/4·라이브 모델 신호 복원·리스크 계량 불변
(TE −0.03%p·β +0.003·turnover +1%·AS −0.4%p)·MaxDD 개선, IC −8% 는 3분할 부호 혼재(순위 아티팩트 제거의 대가로 1분할에 국한).
채택 시 §8 체크리스트(variant 1줄·acceptance allowlist·production 핀·전체 pytest·flip 절), 새 S0′ = 이 런(IR 1.7633·TE 3.64%,
...
### S18.1 측정 결과 — arm 3종 (2026-09-07 16:39~23:06, `outputs/s18_run_chain.bat`, 판정 `scripts/eval_s18_arm.py`, base = s18_s0recert IR 1.6106)

G0 3/3 PASS(빈티지 쌍 워크북 2026-09-04 05:50:52Z / Index 2026-09-07 02:04:19Z 동일, `tg_px_ratio_suspect` 3 arm 모두 0건). 운영: 2번 arm 은 PC 배터리
모드 절전으로 17:01→22:29(CPU 시간 39분)에 걸렸으나 EXIT 0·빈티지 불변 — 결과 유효.

| 항목 | S0′ (s18_s0recert) | **arm 1** 틸트 음자본 마스크 | **arm 2** TG 기저 이벤트 | **arm 3** 정적 집행 |
|---|---:|---:|---:|---:|
| IR (full) | 1.6106 | **1.6373** (+0.027) | **1.7068** (+0.096) | **1.8248** (+0.214) |
| 3분할 ΔIR | — | +0.007 / −0.014 / +0.076 | −0.069 / +0.094 / +0.234 | +0.152 / −0.051 / +0.469 |
| TE | 3.651% | 3.673% | 3.614% | 3.947% |
| turnover (비) | 0.677 | 0.685 (1.012×) | 0.711 (1.050×) | **0.867 (1.280×)** |
| Pictet AS | 19.45% | 19.62% | 19.48% | 21.02% |
| avg_ic | 0.016621 | 0.017723 (+6.6%) | 0.017131 (+3.1%) | 0.016621 (**비트 동일**) |
| 퇴화 | 11/33 | 11/33 | 12/33 | 11/33 |
| MaxDD | −32.1% | −32.3% | −32.1% | −32.9% |
| β | 1.054 | 1.056 | 1.054 | 1.053 |
| E2 do-no-harm | — | PASS 4/4 | PASS 4/4 | **FAIL(turnover 1.28× > 1.25)** |
| 기전 | — | PASS: 음자본 21종 변경 셀 12,370·평균 +0.27 sd | PASS: RTX 2014~19 z −3.95→+0.14 · T −2.43→−0.35 · DELL 2019~21 +5.0→+0.25 · DHR 2014~15 +3.49→+0.15 · z sd 0.925→0.940 | FAIL(회전율 밴드 밖) |
| formal E1 / no-harm | — | FAIL / PASS | FAIL / PASS | FAIL / PASS |
| **flip 후보** | — | **TRUE** (정확성) | **TRUE** (정확성) | **FALSE** |

**해석**: arm 1·2 는 정확성 트랙 flip 후보(기전 완전 작동·리스크 계량 불변, ΔIR 은 관측; §S16.1·§S17.1-A·§S17.3 선례). arm 2 의 1분할 −0.07 은
학습기 초반(RTX·T 의 6~8년 음의 순위 아티팩트 제거 시기)에 국한되고 후반 2분할 양. arm 3 는 예측 비트 동일(순수 집행 노브 확인)이고 ΔIR +0.214
(3분할 부호 혼재)이나 회전율 +28%·TE +0.30%p·AS +1.6%p 로 사전등록 E2 회전율 상한(1.25×)을 넘어 **불채택** — 정적 집행이 "무작위 감속 제거"만이
