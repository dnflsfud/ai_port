...
one_way_tc 0.0010 기준 비용은 0.000849 × 12 × 0.0010 ≈ **1.0bp/yr**이므로
순이득은 +0.028 − 0.010 ≈ **+0.018%p/yr**. IR 1.83 북에서 무의미하다.
...
**검증 주체 고지**: 독립 검증자 에이전트(`verify1350`)를 파견했으나 보고 없이
2회 유휴 전환해 **메인이 V1~V6을 직접 재실행**했다. §S13.48의 독립 검증(결함 3건
적발)과 달리 이번 절의 검증은 **자기검증**이므로, 독립성 측면에서 §S13.48보다
약하다는 점을 명시한다(CLAUDE.md §4 요구의 부분 미충족).
