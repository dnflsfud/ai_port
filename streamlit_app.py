...
def line_fig(df: pd.DataFrame, cols: list[str], title: str, y_title: str = "") -> go.Figure:
    fig = go.Figure()
...
def grouped_bar_fig(df: pd.DataFrame, x: str, y: list[str], title: str) -> go.Figure:
    fig = px.bar(
        df,
        x=x,
        y=y,
...
def fmt_table(df: pd.DataFrame, percent_cols: list[str], digits: int = 2):
...
tabs = st.tabs(["Overview", "Performance", "Contribution", "Risk", "Turnover", "Signals & Gates"])
